	...
	const { x: n, y: o, platform: s, rects: r, elements: a, strategy: l } = i,
		...
		D = Et(
			await s.getClippingRect({
				element:
					(e = await (s.isElement == null ? void 0 : s.isElement(x))) == null ||
					e
						? x
						: x.contextElement ||
...
var ri = (i) => (
	i === void 0 && (i = {}),
	{
		...
		async fn(t) {
			...
			if (
				((V = [...V, { placement: o, overflows: M }]), !M.every((T) => T <= 0))
			) {
				var K, N;
				const T = (((K = s.flip) == null ? void 0 : K.index) || 0) + 1,
					ot = q[T];
				if (
					ot &&
	...
	li = (i) => (
		i === void 0 && (i = {}),
		{
			...
			async fn(t) {
				...
				const x = l.fn({ ...t, [p]: g, [f]: m });
				return {
					...x,
					data: { x: x.x - e, y: x.y - n, enabled: { [p]: r, [f]: a } },
				};
			},
		}
	);
...
function Tt(i) {
	const { overflow: t, overflowX: e, overflowY: n, display: o } = it(i);
	return (
...
function at(i, t, e, n) {
	if (i) {
		...
		do {
			if (
				(t != null &&
					(t[0] === ">" ? i.parentNode === e && we(i, t) : we(i, t))) ||
				(n && i === e)
			)
				return i;
			if (i === e) break;
	...
	_onDragStart: function (t, e) {
		...
		j("setupClone", this),
			...
			(this.cloneId = be(() => {
				j("clone", this),
					!v.eventCanceled &&
						(this.options.removeCloneOnHide || L.insertBefore(R, u),
						this._hideClone(),
						Y({ sortable: this, name: "clone" }));
			})),
			!e && Q(u, s.dragClass, !0),
			e
				? ((ye = !0), (this._loopId = setInterval(this._emulateDragOver, 50)))
				...
				ht(
					{
						...
						target: n,
						completed: y,
						onMove: (ne, Ae) => pe(L, e, u, o, ne, F(ne), t, Ae),
						changed: A,
					},
					tt,
				),
		...
		(Pt = !1),
			...
			t &&
				...
				u &&
					...
					Yt &&
						!Pt &&
						Q(u, W ? W.options.ghostClass : this.options.ghostClass, !1),
					...
					? ((P =
						...
						(C =
							...
							(M.overflowY === "auto" ||
								...
								M.overflowY === "visible")))
					: ((P =
							A < q && (M.overflowX === "auto" || M.overflowX === "scroll")),
						(C =
							O < J && (M.overflowY === "auto" || M.overflowY === "scroll")));
				var N =
						P &&
						(Math.abs(y - o) <= r && V + A < q) - (Math.abs(D - o) <= r && !!V),
					k =
						C &&
						(Math.abs(x - s) <= r && K + O < J) - (Math.abs(m - s) <= r && !!K);
		...
		init() {
			f ||
				...
				this.$watch("state", async (N) => {
					...
					const k = this.getServerState();
					if (k === void 0 || this.getNormalizedState() === k) return;
					this.isLoading = !0;
					const T = await this.$wire.updateTableColumnState(m, C, this.state);
