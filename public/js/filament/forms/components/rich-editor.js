...
be.prototype = {
	...
	update: function (t, e, n) {
		var r = n && n != t ? this.remove(n) : this,
			...
			i = r.content.slice();
		return (
			o == -1 ? i.push(n || t, e) : ((i[o + 1] = e), n && (i[o] = n)), new be(i)
		);
	},
	remove: function (t) {
		var e = this.find(t);
...
function Sc(t, e, n) {
	for (let r = 0; ; r++) {
		...
		if (o.content.size || i.content.size) {
			const s = Sc(o.content, i.content, n + 1);
			if (s != null) return s;
		}
		n += o.nodeSize;
	}
	...
	for (let o = t.childCount, i = e.childCount; ; ) {
		...
		if (s.isText && s.text != l.text) {
			let c = s.text,
				...
				u = c.length,
				f = d.length;
			for (; u > 0 && f > 0 && c[u - 1] == d[f - 1]; ) u--, f--, n--, r--;
			return (
				u &&
					f &&
					u < c.length &&
		...
		if (s.content.size || l.content.size) {
			const c = Cc(s.content, l.content, n - 1, r - 1);
			if (c) return c;
		}
		(n -= a), (r -= a);
	}
...
function ao(t, e) {
	...
	if (n) {
		...
		for (const r in t) if (!(r in e) || !ao(t[r], e[r])) return !1;
...
function Ac(t, e, n, r, o, i) {
	const { index: s, offset: l } = t.findIndex(e),
		a = t.maybeChild(s);
	if (l == e || a.isText)
...
function tr(t, e, n, r) {
	...
	t &&
		((i = t.index(n)),
		t.depth > n ? i++ : t.textOffset && (Kt(t.nodeAfter, r), i++));
	for (let l = i; l < s; l++) Kt(o.child(l), r);
...
function Hm(t) {
	...
	function n(r) {
		...
		const i = (e[r.join(",")] = new Ot(r.indexOf(t.length - 1) > -1));
		for (let s = 0; s < o.length; s++) {
			const l = o[s][1].sort(Lc);
...
function Bc(t, e, n, r) {
	for (const o in e)
		if (!(o in t))
			throw new RangeError(`Unsupported attribute ${o} for ${n} of type ${r}`);
	for (const o in t) t[o].validate && t[o].validate(e[o]);
}
function zc(t, e) {
	const n = Object.create(null);
	if (e) for (const r in e) n[r] = new bs(t, r, e[r]);
	return n;
}
var or = class t {
	constructor(e, n, r) {
...
var bs = class {
		constructor(e, n, r) {
			(this.hasDefault = Object.hasOwn(r, "default")),
		...
		constructor(e) {
			...
			for (const o in this.nodes) {
				...
				const i = this.nodes[o],
					...
					l = i.spec.marks;
				if (
					((i.contentMatch = r[s] || (r[s] = Ot.parse(s, this.nodes))),
			...
			for (const o in this.marks) {
				const i = this.marks[o],
					s = i.spec.excludes;
				i.excluded = s == null ? [i] : s == "" ? [] : gc(this, s.split(" "));
			}
	...
	for (let r = 0; r < e.length; r++) {
		...
		if (i) n.push(i);
			...
			for (const l in t.marks) {
				const a = t.marks[l];
				(o == "_" ||
					(a.spec.group && a.spec.group.split(" ").indexOf(o) > -1)) &&
					n.push((s = a));
			}
...
var Te = class t {
		...
		parse(e, n = {}) {
			const r = new uo(this, n, !1);
			return r.addAll(e, G.none, n.from, n.to), r.finish();
		}
		parseSlice(e, n = {}) {
			const r = new uo(this, n, !0);
		...
		static schemaRules(e) {
			...
			for (const o in e.marks) {
				...
				i &&
					i.forEach((s) => {
						r((s = bc(s))), s.mark || s.ignore || s.clearMark || (s.mark = o);
					});
			}
			for (const o in e.nodes) {
				...
				i &&
					i.forEach((s) => {
						r((s = bc(s))), s.node || s.ignore || s.mark || (s.node = o);
					});
			}
			return n;
		}
		static fromSchema(e) {
			return (
				e.cached.domParser || (e.cached.domParser = new t(e, t.schemaRules(e)))
			);
		}
	},
	Hc = {
	...
	Wm = { head: !0, noscript: !0, object: !0, script: !0, style: !0, title: !0 },
	$c = { ol: !0, ul: !0 },
	ir = 1,
	ws = 2,
	nr = 4;
...
var vn = class {
		...
		finish(e) {
			...
			return (
				!e && this.match && (n = n.append(this.match.fillBefore(v.empty, !0))),
		...
		inlineContext(e) {
			return this.type
				...
				: this.content.length
					...
					: e.parentNode &&
						!Object.hasOwn(Hc, e.parentNode.nodeName.toLowerCase());
		}
	},
	uo = class {
		constructor(e, n, r) {
			...
			o
				...
				: r
					...
					: (i = new vn(e.schema.topNodeType, null, G.none, !0, null, s)),
				(this.nodes = [i]),
		...
		addElementByRule(e, n, r, o) {
			...
			if (n.node)
				if (((s = this.parser.schema.nodes[n.node]), s.isLeaf))
					...
					const a = this.enter(s, n.attrs || null, r, n.preserveWhitespace);
					a && ((i = !0), (r = a));
				}
			else {
		...
		insertNode(e, n, r) {
			...
			if (o) {
				...
				for (const l of o.concat(e.marks))
					(i.type ? i.type.allowsMarkType(l.type) : wc(l.type, e.type)) &&
						(s = l.addToSet(s));
...
var Ze = class t {
	...
	static renderSpec(e, n, r = null, o) {
		return typeof n == "string" ? { dom: e.createTextNode(n) } : lo(e, n, r, o);
	}
	static fromSchema(e) {
		return (
...
function lo(t, e, n, r) {
	...
	for (let u = d; u < e.length; u++) {
		...
		if (f === 0) {
			...
			const { dom: h, contentDOM: p } = lo(t, f, n, r);
			...
			for (let l = 0; l < this.ranges.length; l += 3) {
				...
				if (e <= u) {
					...
					let p = e == (n < 0 ? a : u) ? null : Jm(l / 3, e - a),
						m = e == a ? Uc : e == u ? jc : fo;
					return (n < 0 ? e != a : e != u) && (m |= Kc), new ar(h, m, p);
				}
				o += d - c;
			}
...
var cr = class t {
	...
	de = class {
		...
		static fromJSON(e, n) {
			...
			const r = xs[n.stepType];
			if (!r) throw new RangeError(`No step type ${n.stepType} defined`);
			return r.fromJSON(e, n);
		}
		static jsonID(e, n) {
			if (e in xs) throw new RangeError("Duplicate use of step JSON ID " + e);
...
var ht = class t extends de {
	...
	apply(e) {
		const n = e.slice(this.from, this.to),
			r = new A(
				Ms(n.content, (o) => o.mark(this.mark.removeFromSet(o.marks)), e),
				n.openStart,
				n.openEnd,
			);
...
var ur = class t extends de {
	...
	apply(e) {
		...
		return he.fromReplace(
			e,
			this.pos,
			this.pos + 1,
			new A(v.from(r), 0, n.isLeaf ? 0 : 1),
		);
	}
	invert(e) {
		const n = e.nodeAt(this.pos);
...
var Tn = class t extends de {
	...
	apply(e) {
		const n = e.nodeAt(this.pos);
		if (!n) return he.fail("No node at mark step's position");
		const r = n.type.create(n.attrs, null, this.mark.removeFromSet(n.marks));
		return he.fromReplace(
			e,
			this.pos,
			this.pos + 1,
...
var pe = class t extends de {
	...
	map(e) {
		const n = e.mapResult(this.to, -1),
			r =
				this.from == this.to && t.MAP_BIAS < 0 ? n : e.mapResult(this.from, 1);
	...
	for (let a = 0; a < i.childCount; a++) {
		...
		if (!u) s.push(new pe(l, d, A.empty));
			...
			for (let f = 0; f < c.marks.length; f++)
				n.allowsMarkType(c.marks[f].type) || t.step(new ht(l, d, c.marks[f]));
			...
			return (
				t.step(
					new le(
						...
						u + 1,
						f - 1,
						new A(v.from(r.create(a, null, s.marks)), 0, 0),
						1,
						!0,
					),
				),
...
var vs = class {
		...
		for (let n = 1; n <= 2; n++)
			for (let r = n == 1 ? e : this.unplaced.openStart; r >= 0; r--) {
				let o,
					i = null;
				r
					? ((i = ks(this.unplaced.content, r - 1).firstChild), (o = i.content))
				...
				for (let l = this.depth; l >= 0; l--) {
					...
					if (
						n == 1 &&
						(s
							? c.matchType(s.type) || (u = c.fillBefore(v.from(s), !1))
	...
	placeNodes({
		...
		this.unplaced = p
			? e == 0
				? A.empty
				: new A(sr(s.content, e - 1, 1), e - 1, h < 0 ? s.openEnd : e - 1)
	...
	closeFrontierNode() {
		...
		n.childCount && (this.placed = lr(this.placed, this.frontier.length, n));
	}
};
function sr(t, e, n) {
	return e == 0
		? t.cutByIndex(n, t.childCount)
		: t.replaceChild(0, t.firstChild.copy(sr(t.firstChild.content, e - 1, n)));
}
function lr(t, e, n) {
	return e == 0
		? t.append(n)
...
var Rt = class {
	...
	replace(e, n = e, r = A.empty) {
		const o = fr(this.doc, e, n, r);
		return o && this.step(o), this;
	}
	replaceWith(e, n, r) {
		return this.replace(e, n, new A(v.from(r), 0, 0));
	}
	delete(e, n) {
	...
	I = class {
		...
		replace(e, n = A.empty) {
			...
			for (let l = 0; l < s.length; l++) {
				...
				e.replaceRange(d.map(a.pos), d.map(c.pos), l ? A.empty : n),
					l == 0 && nd(e, i, (r ? r.isInline : o && o.isTextblock) ? -1 : 1);
			}
		}
		replaceWith(e, n) {
			...
			for (let i = 0; i < o.length; i++) {
				...
				i
					? e.deleteRange(c, d)
					: (e.replaceRangeWith(c, d, n), nd(e, r, n.isInline ? -1 : 1));
			}
		}
		static findFrom(e, n, r = !1) {
			...
			for (let i = e.depth - 1; i >= 0; i--) {
				const s =
					n < 0
						? Nn(e.node(0), e.node(i), e.before(i + 1), e.index(i), n, r)
						: Nn(e.node(0), e.node(i), e.after(i + 1), e.index(i) + 1, n, r);
				if (s) return s;
			}
			return null;
		}
	...
	static between(e, n, r) {
		...
		return (
			e.parent.inlineContent ||
				(o == 0
					? (e = n)
					: ((e = (I.findFrom(e, -r, !0) || I.findFrom(e, r, !0)).$anchor),
						e.pos < n.pos != o < 0 && (e = n))),
			new t(e, n)
		);
	}
...
function Nn(t, e, n, r, o, i = !1) {
	...
	for (let s = r - (o > 0 ? 0 : 1); o > 0 ? s < e.childCount : s >= 0; s += o) {
		...
		if (l.isAtom) {
			...
			const a = Nn(t, l, n + o, o < 0 ? l.childCount : 0, o, i);
			if (a) return a;
		}
		n += l.nodeSize * o;
	}
...
var Xt = class {
	...
	On = class t {
		...
		static create(e) {
			const n = new hr(e.doc ? e.doc.type.schema : e.schema, e.plugins),
				r = new t(n);
		...
		reconfigure(e) {
			const n = new hr(this.schema, e.plugins),
				r = n.fields,
				o = new t(n);
			for (let i = 0; i < r.length; i++) {
				const s = r[i].name;
				o[s] = Object.hasOwn(this, s) ? this[s] : r[i].init(e, o);
			}
			return o;
		}
		toJSON(e) {
			const n = { doc: this.doc.toJSON(), selection: this.selection.toJSON() };
			if (
				(this.storedMarks &&
		...
		static fromJSON(e, n, r) {
			...
			const o = new hr(e.schema, e.plugins),
				i = new t(o);
			return (
				o.fields.forEach((s) => {
					if (s.name == "doc") i.doc = te.fromJSON(e.schema, n.doc);
						...
						if (r)
							for (const l in r) {
								const a = r[l],
									c = a.spec.state;
								if (a.key == s.name && c && c.fromJSON && Object.hasOwn(n, l)) {
...
var Rs = (t, e, n) => {
		...
		if (r.parent.content.size == 0 && (Rn(i, "end") || L.isSelectable(i)))
			for (let s = r.depth; ; s--) {
				const l = fr(t.doc, r.before(s), r.after(s), A.empty);
				if (l && l.slice.size < l.to - l.from) {
					if (e) {
						const a = t.tr.step(l);
						a.setSelection(
							Rn(i, "end")
								? I.findFrom(a.doc.resolve(a.mapping.map(o.pos, -1)), -1)
...
var Is = (t, e, n) => {
		...
		const i = o.nodeAfter;
		if (wd(t, o, e, 1)) return !0;
		if (r.parent.content.size == 0 && (Rn(i, "start") || L.isSelectable(i))) {
			const s = fr(t.doc, r.before(), r.after(), A.empty);
...
var $s = (t, e) => {
	...
	Fs = (t, e) => {
		...
		if (n instanceof we || r.parent.inlineContent || o.parent.inlineContent)
			return !1;
		const i = Hs(o.parent.contentMatchAt(o.indexAfter()));
	...
	_s = (t, e) => {
		...
		if (n.depth > 1 && n.after() != n.end(-1)) {
			const i = n.before();
			if (Oe(t.doc, i)) return e && e(t.tr.split(i).scrollIntoView()), !0;
		}
...
function yg(t) {
	return (e, n) => {
		...
		for (let h = r.depth; ; h--)
			if (r.node(h).isBlock) {
				(a = r.end(h) == r.pos + (r.depth - h)),
					...
					(l = Hs(r.node(h - 1).contentMatchAt(r.indexAfter(h - 1))));
		...
		if (
			(f || ((i[0] = l ? { type: l } : null), (f = Oe(d.doc, u, i.length, i))),
			!f)
		)
			return !1;
...
var bd = (t, e) => {
		...
		return i == 0
			? !1
			: ((o = n.before(i)), e && e(t.tr.setSelection(L.create(t.doc, o))), !0);
	},
...
function xg(t, e, n) {
	...
	return !r || !o || !r.type.compatibleContent(o.type)
		...
		: !r.content.size && e.parent.canReplace(i - 1, i)
			...
			: !e.parent.canReplace(i, i + 1) || !(o.isTextblock || Ie(t.doc, e.pos))
				? !1
...
function wd(t, e, n, r) {
	...
	if (
		...
		if (n) {
			let h = e.pos + i.nodeSize,
				p = v.empty;
			for (let y = s.length - 1; y >= 0; y--) p = v.from(s[y].create(null, p));
			p = v.from(o.copy(p));
			const m = t.tr.step(
					new le(e.pos - 1, h, e.pos, h, new A(p, 1, 0), s.length, !0),
				),
	...
	if (c && Rn(i, "start", !0) && Rn(o, "end")) {
		...
		if (h.canReplace(h.childCount, h.childCount, m.content)) {
			if (n) {
				let y = v.empty;
				for (let b = p.length - 1; b >= 0; b--) y = v.from(p[b].copy(y));
				const x = t.tr.step(
					new le(
						e.pos - p.length,
						e.pos + i.nodeSize,
...
function xd(t) {
	return (e, n) => {
		...
		return o.node(i).isTextblock
			? (n &&
					n(e.tr.setSelection(E.create(e.doc, t < 0 ? o.start(i) : o.end(i)))),
				!0)
			: !1;
	};
}
...
function vg(t, e, n, r) {
	...
	i < s &&
		(o.step(
			new le(
				...
				i,
				s,
				new A(v.from(n.create(null, r.parent.copy())), 1, 0),
				1,
				!0,
			),
		),
	...
	return (
		Ie(o.doc, a.pos) && a.nodeBefore.type == a.nodeAfter.type && o.join(a.pos),
		e(o.scrollIntoView()),
		!0
	);
}
...
var me = (t) => {
	...
	Qs = null,
	gt = (t, e, n) => {
		const r = Qs || (Qs = document.createRange());
		return r.setEnd(t, n ?? t.nodeValue.length), r.setStart(t, e || 0), r;
	},
	Tg = () => {
		Qs = null;
	},
	sn = (t, e, n, r) => n && (Md(t, e, n, r, -1) || Md(t, e, n, r, 1)),
...
function Ng(t, e) {
	for (;;) {
		...
		} else if (t.parentNode && !kr(t)) (e = me(t) + 1), (t = t.parentNode);
		else return null;
	}
}
function Og(t, e, n) {
...
var Oo = (t) =>
	t.focusNode && sn(t.focusNode, t.focusOffset, t.anchorNode, t.anchorOffset);
function Qt(t, e) {
	...
	if (t.caretPositionFromPoint)
		try {
			...
			if (r)
				return {
					node: r.offsetNode,
					offset: Math.min(Be(r.offsetNode), r.offset),
				};
		} catch {}
	if (t.caretRangeFromPoint) {
		...
		if (r)
			return {
				...
				offset: Math.min(Be(r.startContainer), r.startOffset),
			};
	}
}
...
function au(t, e, n) {
	const {
		...
		s = Sr || ze;
	if (r.nodeType == 3)
		if (s && (jg.test(r.nodeValue) || (n < 0 ? !o : o == r.nodeValue.length))) {
			const a = Lt(gt(r, o, o), n);
			if (ze && o && /\s/.test(r.nodeValue[o - 1]) && o < r.nodeValue.length) {
				const c = Lt(gt(r, o - 1, o - 1), -1);
				if (c.top == a.top) {
					const d = Lt(gt(r, o, o + 1), -1);
...
var He = 0,
	...
	ln = class {
		...
		domFromPos(e, n) {
			...
			for (
				let i;
				r && !(i = this.children[r - 1]).size && i instanceof xo && i.side >= 0;
				r--
			);
			if (n <= 0) {
				let i,
...
function Dd(t, e, n, r, o) {
	fu(r, e, t);
	const i = new Bt(void 0, t, e, n, r, r, r);
...
var ko = class t extends Bt {
		...
		slice(e, n, r) {
			...
			return new t(this.parent, o, this.outerDeco, this.innerDeco, i, i);
		}
		markDirty(e, n) {
			super.markDirty(e, n),
...
function du(t, e, n) {
	...
	for (let i = 0; i < e.length; i++) {
		...
		if (s instanceof tn) {
			...
			du(s.contentDOM, s.children, n), (r = a ? a.nextSibling : t.firstChild);
		}
	}
	for (; r; ) (r = Ld(r)), (o = !0);
...
var sl = class {
	...
	syncToMarks(e, n, r, o) {
		...
		for (; s < e.length; ) {
			...
			if (a < 0 && this.index < this.top.children.length) {
				const d = this.top.children[this.index];
				d instanceof tn &&
					d.dirty != Ue &&
					...
					!this.isLocked(d.dom) &&
					d.spec.update(e[s]) &&
					((d.mark = e[s]), (a = this.index), (this.changed = !0));
			}
			if (a > -1)
				a > this.index &&
				...
				const d = tn.create(this.top, e[s], n, r);
			...
			if (a instanceof Bt) {
				...
				if (!f && (u = this.recreateWrapper(a, e, n, r, o, s)))
					return (
						...
						u.contentDOM &&
							((u.dirty = Zt), u.updateChildren(o, s + 1), (u.dirty = He)),
						(this.changed = !0),
						this.index++,
						!0
					);
	...
	recreateWrapper(e, n, r, o, i, s) {
		if (
			...
			return null;
		const l = Bt.create(this.top, n, r, o, i, s);
		if (l.contentDOM) {
	...
	addNode(e, n, r, o, i) {
		const s = Bt.create(this.top, e, n, r, o, i);
	...
	addTextblockHacks() {
		let e = this.top.children[this.index - 1],
			n = this.top;
		for (; e instanceof tn; ) (n = e), (e = n.children[n.children.length - 1]);
		(!e ||
			!(e instanceof ko) ||
			/\n$/.test(e.node.text) ||
	...
	addHackNode(e, n) {
		if (
			...
			const o = new So(this.top, [], r, null);
			n != this.top
				? n.children.push(o)
...
function Qg(t, e, n, r) {
	...
	for (let c = 0; ; ) {
		...
		const m = f.isInline && !f.isLeaf ? l.filter((g) => !g.inline) : l.slice();
...
function wl(t, e = null) {
	...
	const d = r.resolve(a);
	if (!c) {
		const u = e == "pointer" || (t.state.selection.head < l.pos && !i) ? 1 : -1;
		c = xl(t, d, l, u);
	}
	return c;
}
function hu(t) {
	return t.editable
		? t.hasFocus()
		: mu(t) && document.activeElement && document.activeElement.contains(t.dom);
}
function wt(t, e = !1) {
...
function $d(t, e, n) {
	...
	if (r instanceof E)
		if (n.indexOf("s") > -1) {
			...
			const s = t.state.doc.resolve(o.pos + i.nodeSize * (e < 0 ? -1 : 1));
			return It(t, new E(r.$anchor, s));
		} else if (r.empty) {
			if (t.endOfTextblock(e > 0 ? "forward" : "backward")) {
				const o = al(t.state, e);
				...
				return i.isAtom || ((s = t.docView.descAt(l)) && !s.contentDOM)
					? L.isSelectable(i)
						...
						: Sr
							? It(t, new E(t.state.doc.resolve(e < 0 ? l : l + i.nodeSize)))
							: !1
					: !1;
			}
		} else return !1;
...
function oy(t) {
	...
	let o,
		...
		s = !1;
	for (
		ze && n.nodeType == 1 && r < vo(n) && gr(n.childNodes[r], -1) && (s = !0);
		;
	)
		if (r > 0) {
...
function dy(t, e) {
	...
	if (n == 37 || (Pe && n == 66 && r == "c")) {
		const o = n == 37 ? (Fd(t, t.state.selection.from) == "ltr" ? -1 : 1) : -1;
		return $d(t, o, r) || Ln(t, o);
	} else if (n == 39 || (Pe && n == 70 && r == "c")) {
		const o = n == 39 ? (Fd(t, t.state.selection.from) == "ltr" ? 1 : -1) : 1;
		return $d(t, o, r) || Ln(t, o);
	} else {
		if (n == 38 || (Pe && n == 80 && r == "c"))
...
function kl(t, e) {
	...
	const s = t.someProp("clipboardSerializer") || Ze.fromSchema(t.state.schema),
		l = ku(),
...
`,
	...
	else if (((l = A.maxOpen(fy(l.content, o), !0)), l.openStart || l.openEnd)) {
		let u = 0,
			f = 0;
		for (
		...
		if (
			(t.forEach((l) => {
				...
				let a = o.findWrapping(l.type),
					c;
				if (!a) return (s = null);
				if ((c = s.length && i.length && bu(a, i, l, s[s.length - 1], 0)))
					s[s.length - 1] = c;
				else {
...
function bu(t, e, n, r, o) {
	if (o < t.length && o < e.length && t[o] == e[o]) {
		const i = bu(t, e, n, r.lastChild, o + 1);
...
function wu(t, e) {
	if (e == 0) return t;
	const n = t.content.replaceChild(t.childCount - 1, wu(t.lastChild, e - 1)),
...
function py(t) {
	...
	for (let s = 0; s < n.styleSheets.length; s++) {
		const l = n.styleSheets[s];
		for (let a = 0; a < l.rules.length; a++) {
			const c = l.rules[a];
			if (c instanceof CSSStyleRule) {
				const d = r.querySelectorAll(c.selectorText);
	...
	for (const e in Ce) {
		...
		t.dom.addEventListener(
			...
			(t.input.eventHandlers[e] = (r) => {
				xy(t, r) && !Cl(t, r) && (t.editable || !(r.type in ve)) && n(t, r);
			}),
	...
	t.someProp("handleDOMEvents", (e) => {
		for (const n in e)
			t.input.eventHandlers[n] ||
				t.dom.addEventListener(n, (t.input.eventHandlers[n] = (r) => Cl(t, r)));
	});
}
function Cl(t, e) {
...
function Ey(t, e, n) {
	...
	return r
		? (vr(t, r, "pointer"),
			r instanceof E && o.eq(t.state.doc) && (t.input.mouseDown = new pl(t, r)),
			!0)
		: !1;
}
function Su(t, e, n) {
...
var Mo = class {
		...
		move(e) {
			...
			this.view.root.removeEventListener("mouseup", this.up),
				...
				this.view.input.mouseDown == this && (this.view.input.mouseDown = null);
		}
		delaySelUpdate() {
			return !1;
		}
	...
	hl = class extends Mo {
		...
		move(e) {
			this.updateAllowDefault(e), bt(this.view, "pointer"), super.move(e);
		}
	...
	pl = class extends Mo {
		...
		move(e) {
			...
			const n = this.view.posAtCoords(Cr(e)),
				r = n && Su(this.view, n.inside, !1);
			if (!r) return;
			const { doc: o } = this.view.state,
...
function Ry(t) {
	...
	const n = Eg(e.focusNode, e.focusOffset),
...
function yr(t, e = !1) {
	if (!(yt && t.domObserver.flushingSoon >= 0)) {
		if (
			(t.domObserver.forceFlush(), Tu(t), e || (t.docView && t.docView.dirty))
		) {
			const n = wl(t),
				r = t.state.selection;
			return (
...
Ce.copy = ve.cut = (t, e) => {
	const n = e,
		...
		o = n.type == "cut";
	if (r.empty) return;
	const i = br ? null : n.clipboardData,
		s = r.content(),
		{ dom: l, text: a } = kl(t, s);
	i
		? (n.preventDefault(),
...
function Iy(t, e) {
	...
	setTimeout(() => {
		t.focus(),
			...
			n ? wr(t, r.value, null, o, e) : wr(t, r.textContent, r.innerHTML, o, e);
	}, 50);
}
function wr(t, e, n, r, o) {
	const i = Sl(t, e, n, r, t.state.selection.$from);
...
ve.paste = (t, e) => {
	...
	const r = br ? null : n.clipboardData,
...
var Ao = class t {
	...
	nn = class t {
		...
		eq(e) {
			return (
				this == e ||
				(e instanceof t && xr(this.attrs, e.attrs) && xr(this.spec, e.spec))
			);
		}
		static is(e) {
	...
	X = class t {
		constructor(e, n) {
			(this.local = e.length ? e : In), (this.children = n.length ? n : In);
		}
		static create(e, n) {
			...
			e.forEach((l, a) => {
				...
				if ((d = Ou(n, l, c))) {
					...
					o[i] == a
						...
						: o.splice(i, 0, a, a + l.nodeSize, No(d, l, c + 1, rn)),
						(i += 3);
				}
			});
			const s = Nu(i ? Ru(n) : n, -r);
			for (let l = 0; l < s.length; l++)
...
var xe = X.empty,
	Eo = class t {
		...
		map(e, n) {
			const r = this.members.map((o) => o.map(e, n, rn));
			return t.from(r);
		}
		forChild(e, n) {
			...
			for (let o = 0; o < this.members.length; o++) {
				...
				i != xe && (i instanceof t ? (r = r.concat(i.members)) : r.push(i));
			}
			return t.from(r);
		}
		eq(e) {
				...
				default:
					return new t(
						e.every((n) => n instanceof X)
							? e
							: e.reduce(
									(n, r) => n.concat(r instanceof X ? r : r.members),
									[],
								),
					);
			}
...
function No(t, e, n, r) {
	...
	e.forEach((l, a) => {
		...
		if (c) {
			i = !0;
			const d = No(c, l, n + a + 1, r);
			d != xe && o.push(a, a + l.nodeSize, d);
		}
	});
	const s = Nu(i ? Ru(t) : t, -n).sort(on);
	for (let l = 0; l < s.length; l++)
...
function Xs(t) {
	...
	return (
		...
		t.cursorWrapper && e.push(X.create(t.state.doc, [t.cursorWrapper.deco])),
		Eo.from(e)
	);
}
var $y = {
	...
	yl = class {
		...
		start() {
			this.observer &&
				(this.observer.takeRecords(), this.observer.observe(this.view.dom, $y)),
				this.onCharData &&
		...
		flush() {
			...
			if (
				...
				(e.input.lastKeyCode == 8 ||
					...
					(ue &&
						(e.composing || e.input.compositionEndedAt > Date.now() - 50) &&
						n.some((d) => d.type == "childList" && d.removedNodes.length)))
			) {
				for (const d of a)
				...
				if (d.length == 2) {
					...
					for (const f of d) {
						...
						h && h.nodeName == "LI" && (!u || Wy(e, u) != h) && f.remove();
					}
				}
			}
			let c = null;
		...
		registerMutation(e, n) {
			...
			if (e.type == "childList") {
				...
				if (Ae && Pt <= 11 && e.addedNodes.length)
					for (let d = 0; d < e.addedNodes.length; d++) {
						const { previousSibling: u, nextSibling: f } = e.addedNodes[d];
						(!u || Array.prototype.indexOf.call(e.addedNodes, u) < 0) &&
							(o = u),
							(!f || Array.prototype.indexOf.call(e.addedNodes, f) < 0) &&
								(i = f);
					}
				const s = o && o.parentNode == e.target ? me(o) + 1 : 0,
					l = r.localPosFromDOM(e.target, s, -1),
					a =
						i && i.parentNode == e.target ? me(i) : e.target.childNodes.length,
...
function jy(t, e) {
	var n;
	const { focusNode: r, focusOffset: o } = t.domSelectionRange();
	for (const i of e)
		if (
			((n = i.parentNode) === null || n === void 0 ? void 0 : n.nodeName) ==
			"TR"
		) {
			let s = i.nextSibling;
...
function Jy(t, e, n, r, o) {
	...
	if (!p)
		if (
			...
			if (c.sel) {
				const S = Gd(t, t.state.doc, c.sel);
				if (S && !S.eq(t.state.selection)) {
					const R = t.state.tr.setSelection(S);
	...
	if (x)
		if (m.pos == g.pos) {
			...
			const S = k(t.state.tr.delete(b, w)),
			...
			const S = k(t.state.tr);
			C.type == "add" ? S.addMark(b, w, C.mark) : S.removeMark(b, w, C.mark),
				t.dispatch(S);
		} else if (
			m.parent.child(m.index()).isText &&
			m.index() == g.index() - (g.textOffset ? 0 : 1)
		) {
			const S = m.parent.textBetween(m.parentOffset, g.parentOffset),
				R = () => k(t.state.tr.insertText(S, b, w));
			t.someProp("handleTextInput", (T) => T(t, b, w, S, R)) || t.dispatch(R());
		} else t.dispatch(k());
	else t.dispatch(k());
}
function Gd(t, e, n) {
...
function Yy(t, e, n, r, o) {
	...
	if (o == "end") {
		const d = Math.max(0, i - Math.min(a, c));
		r -= a + d - i;
	}
	if (a < i && s < l) {
		const d = r <= i && r >= a ? i - r : 0;
		(i -= d), (c = i + (c - a)), (a = i);
	} else if (c < i) {
		const d = r <= i && r >= c ? i - r : 0;
		(i -= d), (a = i + (a - c)), (c = i);
	}
	...
	zn = class {
		constructor(e, n) {
			(this._root = null),
				...
				(this.docView = Dd(this.state.doc, Xd(this), Xs(this), this.dom, this)),
				(this.domObserver = new yl(this, (r, o, i, s) => Jy(this, r, o, i, s))),
		...
		update(e) {
			...
			const n = this._props;
			(this._props = e),
				e.plugins && (e.plugins.forEach(eu), (this.directPlugins = e.plugins)),
		...
		updateStateInner(e, n) {
			...
			let o = this.state,
				i = !1,
				s = !1;
			e.storedMarks && this.composing && (Tu(this), (s = !0)), (this.state = e);
			...
			(l || n.handleDOMEvents != this._props.handleDOMEvents) && fl(this),
				(this.editable = Qd(this)),
				Yd(this);
			const a = Xs(this),
				c = Xd(this),
			...
			if (s) {
				...
				!(
					...
					? wt(this, h)
					: (pu(this, e.selection), this.domObserver.setCurSelection()),
					this.domObserver.start();
			}
		...
		scrollToSelection() {
			...
			if (!(!e || !this.dom.contains(e.nodeType == 1 ? e : e.parentNode))) {
				if (!this.someProp("handleScrollToSelection", (n) => n(this)))
					...
					} else Ad(this, this.coordsAtPos(this.state.selection.head, 1), e);
			}
		}
		destroyPluginViews() {
			let e;
		...
		updateDraggedNode(e, n) {
			...
			if (
				...
				const i = r.from + (this.state.doc.content.size - n.doc.content.size);
				(i > 0 &&
...
function Zd(t) {
	const e = Object.create(null);
	function n(r) {
		for (const o in r) Object.hasOwn(e, o) || (e[o] = r[o]);
	}
...
var ae;
for (ae = 65; ae <= 90; ae++)
	(xt[ae] = String.fromCharCode(ae + 32)), (Do[ae] = String.fromCharCode(ae));
var ae;
for (Ro in xt) Object.hasOwn(Do, Ro) || (Do[Ro] = xt[Ro]);
var Ro;
function Du(t) {
	var e =
...
var gb =
	...
	xb =
		...
		({ state: t, dispatch: e }) => {
			...
			if (e) {
				const n = t.tr,
					{ ranges: r } = t.selection,
					o = n.steps.length;
				r.forEach((i) => {
					const s = n.mapping.slice(o),
...
function Vu(t, e, n = {}) {
	return t.find(
		(r) =>
			r.type === e &&
			Nr(Object.fromEntries(Object.keys(n).map((o) => [o, r.attrs[o]])), n),
	);
}
function Pu(t, e, n = {}) {
	return !!Vu(t, e, n);
...
var Cb =
		...
		({ tr: n, state: r, dispatch: o }) => {
			const i = nt(t, r.schema),
				{ doc: s, selection: l } = n,
				{ $from: a, from: c, to: d } = l;
			if (o) {
				const u = Fo(a, i, e);
				if (u && u.from <= c && u.to >= d) {
					const f = E.create(s, u.from, u.to);
					n.setSelection(f);
				}
			}
			return !0;
...
var Mb =
		...
		({ editor: n, view: r, tr: o, dispatch: i }) => {
			...
			const s = () => {
				(an() || Or()) && r.dom.focus(),
					Wu() && !an() && !Or() && r.dom.focus({ preventScroll: !0 }),
					requestAnimationFrame(() => {
						n.isDestroyed ||
			...
			const l = Bo(o.doc, t) || n.state.selection,
							...
							parseDOM: [
								{
									tag: "*",
									getAttrs: (c) => (
										(s = !0), (l = typeof c == "string" ? c : c.outerHTML), null
									),
								},
							],
						},
		...
		({ tr: r, dispatch: o, editor: i }) => {
			...
			if (o) {
				...
				if (
					...
					try {
						cn(e, i.schema, { parseOptions: c, errorOnInvalidContent: !0 });
					} catch (g) {
						a(g);
					}
				try {
				...
				if (f) {
					...
					const g = r.doc.resolve(d),
						...
						w = y.content.size > 0;
					x && b && w && h && (d = Math.max(0, d - 1)), r.replaceWith(d, u, m);
				}
...
var Ob =
		...
		({ tr: e, dispatch: n, editor: r }) => {
			...
			typeof o == "number"
				...
				: o
					? (a = o)
					: (a = e.selection.$from);
			const c = un(a.parent.contentMatchAt(a.index()));
			if (!c) return !1;
			let d = Object.keys(c.spec.attrs || {}),
				u = i
					? Object.fromEntries(Object.entries(i).filter(([h]) => d.includes(h)))
					: {},
				f;
			if (s) {
				const h = cn(s, r.schema);
				f = c.createAndFill(u, h);
			} else f = c.createAndFill(u);
			return f
				? (n && (e.insert(a.pos, f), l && Bl(e, e.steps.length - 1, -1)), !0)
				: !1;
		},
	Rb =
		() =>
	...
	Pb =
		...
		({ state: t, dispatch: e, tr: n }) => {
			try {
				const r = Gt(t.doc, t.selection.$from.pos, -1);
			...
			return (
				n.selection.ranges.forEach((c) => {
					r.doc.nodesBetween(c.$from.pos, c.$to.pos, (d, u) => {
						i &&
							i === d.type &&
							((a = !0), o && n.setNodeMarkup(u, void 0, Rl(d.attrs, e))),
							s &&
								d.marks.length &&
								d.marks.forEach((f) => {
									s === f.type &&
										((a = !0),
										o &&
											n.addMark(u, u + d.nodeSize, s.create(Rl(f.attrs, e))));
								});
					});
				}),
				a
...
var Zb =
	(
		t,
		{ errorOnInvalidContent: e, emitUpdate: n = !0, parseOptions: r = {} } = {},
	) =>
	({ editor: o, tr: i, dispatch: s, commands: l }) => {
		const { doc: a } = i;
		if (r.preserveWhitespace !== "full") {
			const c = zo(t, o.schema, r, {
				errorOnInvalidContent: e ?? o.options.enableContentCheck,
			});
			return (
				s && i.replaceWith(0, a.content.size, c).setMeta("preventUpdate", !n),
				!0
			);
		}
		return (
...
function P(t, e, n) {
	return t.config[e] === void 0 && t.parent
		...
		: typeof t.config[e] == "function"
			? t.config[e].bind({ ...n, parent: t.parent ? P(t.parent, e, n) : null })
			: t.config[e];
}
function Wo(t) {
	return t
...
function Ir(t, e) {
	const n = Ze.fromSchema(e).serializeFragment(t),
		...
		l = Object.fromEntries(
			o.map((c) => {
				const d = r.filter((y) => y.type === c.name),
					...
					f = t.reduce((y, x) => {
						const b = P(x, "extendNodeSchema", u);
						return { ...y, ...(b ? b(c) : {}) };
					}, {}),
					h = zu({
						...f,
						...
						linebreakReplacement: q(P(c, "linebreakReplacement", u)),
				...
				m && (h.toDOM = (y) => m({ node: y, HTMLAttributes: kt(y, d) }));
		...
		a = Object.fromEntries(
			i.map((c) => {
				const d = r.filter((g) => g.type === c.name),
					...
					f = t.reduce((g, y) => {
						const x = P(y, "extendMarkSchema", u);
						return { ...g, ...(x ? x(c) : {}) };
					}, {}),
					h = zu({
						...f,
				...
				const m = P(c, "renderHTML", u);
				return (
					m && (h.toDOM = (g) => m({ mark: g, HTMLAttributes: kt(g, d) })),
					[c.name, h]
				);
			}),
		);
...
`,
	...
	return (
		t.nodesBetween(r, o, (a, c, d, u) => {
			...
			if (h)
				return (
					d && (l += h({ node: a, pos: c, parent: d, index: u, range: e })), !1
				);
			a.isText &&
				(l +=
					(f = a?.text) == null ? void 0 : f.slice(Math.max(r, c) - c, o - c));
		}),
		l
	);
}
...
function qo(t, e) {
	const n = Lr(typeof e == "string" ? e : e.name, t.schema);
...
function Gu(t, e = JSON.stringify) {
	...
	return t.filter((r) => {
		const o = e(r);
		return Object.hasOwn(n, o) ? !1 : (n[o] = !0);
	});
}
function o0(t) {
	const e = Gu(t);
		...
		(o.forEach(({ $from: u, $to: f }) => {
			...
			t.doc.nodesBetween(h, p, (m, g) => {
				...
				const y = Math.max(h, g),
					...
					b = x - y;
				(s += b), l.push(...m.marks.map((w) => ({ mark: w, from: y, to: x })));
			});
		}),
		s === 0)
	)
...
function hn(t, { checkChildren: e = !0, ignoreWhitespace: n = !1 } = {}) {
	...
	if (e) {
		...
		return (
			t.content.forEach((i) => {
				o !== !1 &&
					(hn(i, { ignoreWhitespace: n, checkChildren: e }) || (o = !1));
			}),
			o
		);
	}
			...
			t.doc.nodesBetween(l.pos, a.pos, (d, u, f) => {
				...
				if (d.isInline) {
					const h = !f || f.type.allowsMarkType(n),
						p =
							!!n.isInSet(d.marks) || !d.marks.some((m) => m.type.excludes(n));
					c = h && p;
				}
				return !c;
			}),
...
var m0 =
		...
		({ tr: e, state: n, dispatch: r, editor: o }) => {
			...
			if (
				(!h &&
					!p &&
					Oe(e.doc, e.mapping.map(l.pos), 1, f ? [{ type: f }] : void 0) &&
				...
				t && $u(n, o.extensionManager.splittableMarks), e.scrollIntoView();
			}
			return p;
		},
	g0 =
		...
		({ tr: n, state: r, dispatch: o, editor: i }) => {
			...
			if (o) {
				...
				if ((n.split(a.pos, 2, g).scrollIntoView(), !w || !o)) return !0;
				const k = w.filter((C) => b.includes(C.type.name));
				n.ensureMarks(k);
			}
			return !0;
		};
...
var Nl = (t, e) => {
		const n = qe((s) => s.type === e)(t.selection);
		if (!n) return !0;
...
var b0 =
		...
		({
			...
			if (!b) return !1;
			const k = qe((B) => Po(B.type.name, u))(m),
				C = m.from === 0 && m.to === s.doc.content.size,
				S = s.doc.content.content,
				R = S.length === 1 ? S[0] : null,
				T = C && R && Po(R.type.name, u) ? { node: R, pos: 0, depth: 0 } : null,
				O = k ?? T,
					...
					return C && F
						? a()
								.command(({ tr: B, dispatch: _ }) => {
									const V = y0(B);
									return V ? (B.setSelection(V), _ && _(B), !0) : !1;
								})
								.liftListItem(p)
								.run()
						: c.liftListItem(p);
		...
		({ tr: e, dispatch: n, editor: r }) => {
			...
			if (n) {
				const c = Object.values(r.schema.marks).filter(
					(d) => o || !a.includes(d.name),
				);
				l.forEach((d) => {
					for (const u of c) e.removeMark(d.$from.pos, d.$to.pos, u);
				});
			}
			return !0;
		},
											...
											g.marks.forEach((x) => {
												if (s === x.type && ((a = !0), o)) {
													const b = Math.max(y, d),
														w = Math.min(y + g.nodeSize, u);
													n.addMark(b, w, s.create({ ...x.attrs, ...e }));
												}
											}));
							}),
						h &&
							(f !== void 0 &&
								o &&
								n.setNodeMarkup(f, void 0, { ...h.attrs, ...e }),
							s &&
								h.marks.length &&
								h.marks.forEach((g) => {
									s === g.type &&
...
function rf(t, e) {
	...
	return n.length !== r.length
		? !1
		: n.every((o) => Object.hasOwn(e, o) && Object.is(t[o], e[o]));
}
function Xl(t, e) {
	const { selection: n } = t,
		{ $from: r } = n;
				...
				for (S.lastIndex = 0; ; ) {
					...
					if (!((p = T[2]) != null && p.endsWith(":::"))) {
						if (z) w += 1;
										...
										_ = h.blockTokens(F),
											_.forEach((V) => {
												V.text &&
													...
													(V.tokens = h.inlineTokens(V.text));
											});
										_.length > 0;
									) {
							...
							return { type: e, raw: B, attributes: b, content: C, tokens: _ };
						}
					}
				}
			},
...
`;
		...
		for (d += 1; d < l.length; ) {
			const k = l[d];
			if (k.trim() === "") {
				const S = l.slice(d + 1).findIndex((O) => O.trim() !== "");
				if (S === -1) break;
				if (
					(((o = (r = l[d + 1 + S].match(/^(\s*)/)) == null ? void 0 : r[1]) ==
					null
						? void 0
						: o.length) || 0) > m
				) {
	...
	return (
		s &&
			...
			s.forEach((c, d) => {
				var u, f;
				const h =
					(f = (u = e.renderChild) == null ? void 0 : u.call(e, c, d + 1)) !=
					null
						? f
						: e.renderChildren([c]);
				if (h != null) {
...
function z0(t, e) {
	...
	return t.every((r) => {
		const o = _u(r),
			i = e.findIndex((s, l) => !n[l] && o === _u(s) && rf(r.attrs, s.attrs));
	...
	return (
		s.forEach((f) => {
			...
			const m = a.state.tr,
				...
				{ commands: x, chain: b, can: w } = new Dr({ editor: n, state: g });
			f.handler({
				state: g,
				range: y,
				match: h,
				...
				!m.steps.length ||
				(f.undoable && m.setMeta(l, { transform: m, from: r, to: o, text: i }),
				a.dispatch(m),
				(d = !0));
		}),
		d
				...
				apply(o, i, s) {
					...
					return (
						a &&
							setTimeout(() => {
								let { text: d } = a;
								typeof d == "string" ? (d = d) : (d = Ir(v.from(d), s.schema));
								const { from: u } = a,
									f = u + d.length;
								Lo({ editor: e, from: u, to: f, text: d, rules: n, plugin: r });
							}),
...
var Yo = class {
		...
		get storage() {
			return {
				...q(P(this, "addStorage", { name: this.name, options: this.options })),
			};
		}
		configure(t = {}) {
			const e = this.extend({
			...
			return (
				(e.name = this.name), (e.parent = this.parent), (this.child = null), e
			);
		}
		extend(t = {}) {
		...
		(d) =>
			new D({
				...
				props: {
					handleDOMEvents: {
						...
						paste: (u, f) => {
							var h;
							const p =
								(h = f.clipboardData) == null ? void 0 : h.getData("text/html");
				...
				appendTransaction: (u, f, h) => {
					...
					if (x) {
						let { text: k } = y;
						typeof k == "string" ? (k = k) : (k = Ir(v.from(k), h.schema));
						const { from: C } = y,
							S = C + k.length,
							R = V0(k);
						return a({ rule: d, state: h, from: C, to: { b: S }, pasteEvt: R });
					}
					const b = f.doc.content.findDiffStart(h.doc.content),
						w = f.doc.content.findDiffEnd(h.doc.content);
					if (!(!sf(b) || !w || b === w.b))
						return a({ rule: d, state: h, from: b, to: w, pasteEvt: s });
				},
			}),
	);
}
	...
	get plugins() {
		...
		return Fn([...this.extensions].reverse()).flatMap((r) => {
			...
			if (
				(r.type === "mark" &&
					P(r, "exitable", o) &&
					(l.ArrowRight = () => ee.handleExit({ editor: t, mark: r })),
				s)
			) {
			...
			const a = Lu(l);
			i.push(a);
			const c = P(r, "addInputRules", o);
			if (Ll(r, t.options.enableInputRules) && c) {
				const f = c();
				if (f && f.length) {
					const h = df({ editor: t, rules: f }),
						p = Array.isArray(h) ? h : [h];
					i.push(...p);
				}
			}
			const d = P(r, "addPasteRules", o);
			if (Ll(r, t.options.enablePasteRules) && d) {
				const f = d();
				if (f && f.length) {
					const h = ff({ editor: t, rules: f });
					i.push(...h);
				}
			}
			const u = P(r, "addProseMirrorPlugins", o);
			if (u) {
				const f = u();
				i.push(...f);
			}
...
`,
				...
				new D({
					...
					appendTransaction: (t, e, n) => {
						...
						const f = n.tr,
							...
							{ commands: p } = new Dr({ editor: this.editor, state: h });
	...
	Mf = j.create({
		...
		addGlobalAttributes() {
			...
			const { nodeExtensions: t } = dn(this.extensions);
			return [
				{
		...
		get children() {
			...
			return (
				this.node.content.forEach((n, r) => {
					...
					const c = new Ar(a, this.editor, o, o || s ? n : null);
	...
	j0 = class extends N0 {
		constructor(t = {}) {
			super(),
				...
				(this.utils = { getUpdatedPosition: Zu, createMappablePosition: ef }),
				this.setOptions(t),
			...
			if (!this.editorState) {
				const n = Bo(e, this.options.autofocus);
			...
			const i = [
				...(this.options.enableCoreExtensions
					? [
							...
							Cf,
							yf,
							Mf.configure({ direction: this.options.textDirection }),
						].filter((s) =>
		...
		createDoc() {
			let t;
			try {
				t = zo(this.options.content, this.schema, this.options.parseOptions, {
		...
		dispatchTransaction(t) {
			...
			l && this.emit("focus", { editor: this, event: l.event, transaction: s }),
				a &&
					this.emit("blur", { editor: this, event: a.event, transaction: s }),
				!(
...
`,
		...
		$nodes(t, e) {
			var n;
			return (
				((n = this.$doc) == null ? void 0 : n.querySelectorAll(t, e)) || null
			);
		}
		$pos(t) {
			const e = this.state.doc.resolve(t),
				n =
					t > 0 && e.nodeAfter && !e.nodeAfter.isText && e.nodeAfter.isAtom
						? e.nodeAfter
						: null;
...
var q0 = (t, e) => {
	...
	X0 = class {
		...
		onDragStart(t) {
			...
			const g = L.create(a.state.doc, m),
...
function Y0(t) {
	return new jn({
		...
		handler({ match: e, chain: n, range: r, pasteEvent: o }) {
			const i = q(t.getAttributes, void 0, e, o),
				s = q(t.getContent, void 0, i);
			if (i === !1 || i === null) return null;
			const l = { type: t.type.name, attrs: i };
			s && (l.content = s),
...
var ta = class {
	...
	dragover(e) {
		...
		const n = this.editorView.posAtCoords({ left: e.clientX, top: e.clientY }),
...
function ew(t) {
	for (let e = t.depth; e >= 0; e--) {
		...
		for (let o = r.child(n - 1); ; o = o.lastChild) {
			if ((o.childCount == 0 && !o.inlineContent) || Nf(o.type)) return !0;
...
function ti(t, e) {
	...
	return (r, o, i) => {
		...
		const c = ce.findGapCursorFrom(l, e, a);
...
function ow(t, e) {
	...
	return i.setSelection(E.near(i.doc.resolve(n.pos + 1))), t.dispatch(i), !1;
}
function iw(t) {
...
ye.prototype.map = function (e, n, r) {
	...
	var o = [];
	return this.forEach((i, s) => o.push(e(i, s)), n, r), o;
};
ye.from = (e) => (e instanceof ye ? e : e && e.length ? new Rf(e) : ye.empty);
var Rf = ((t) => {
	function e(r) {
		...
		return (
			...
			(e.prototype.forEachInvertedInner = function (r, o, i, s) {
				...
				if (
					...
					(i < l &&
						this.left.forEachInvertedInner(r, Math.min(o, l), i, s) === !1)
				)
					return !1;
			}),
...
var lw = 500,
	mn = class t {
					...
					(u, f) => {
						...
						if (o) {
							...
							h &&
								...
								c.push(new rt(p, void 0, void 0, c.length + d.length))),
								i--,
								p && o.appendMap(p, i);
		...
		addTransform(e, n, r, o) {
			...
			const c = s - r.depth;
			return c > cw && ((l = aw(l, c)), (s -= c)), new t(l.append(i), s);
		}
		remapping(e, n) {
			const r = new cr();
			return (
		...
		addMaps(e) {
			return this.eventCount == 0
				? this
				: new t(this.items.append(e.map((n) => new rt(n))), this.eventCount);
		}
		rebased(e, n) {
					...
					(s, l) => {
						if (l >= e) o.push(s), s.selection && i++;
							...
							if ((r--, c && n.appendMap(c, r), a)) {
								const d = s.selection && s.selection.map(n.slice(r));
								d && i++;
...
function dw(t, e, n, r) {
	...
	if (
		...
		return new ot(
			t.done.addTransform(n, a ? e.selection.getBookmark() : void 0, r, ri(e)),
			mn.empty,
			c,
			n.time,
			l ?? t.prevComposition,
...
function fw(t, e, n) {
	...
	return i.transform.setSelection(s).setMeta(pn, { redo: n, historyState: a });
}
var ia = !1,
	Lf = null;
function ri(t) {
		...
		new D({
			...
			props: {
				handleDOMEvents: {
					beforeinput(e, n) {
						const r = n.inputType,
							o = r == "historyUndo" ? sa : r == "historyRedo" ? la : null;
						return !o || !e.editable
							? !1
...
var Qv = j.create({
	...
	Hf = j.create({
		...
		extendNodeSchema(t) {
			...
			return {
				allowGapCursor: (e = q(P(t, "allowGapCursor", n))) != null ? e : null,
			};
		},
	}),
	$f = "placeholder",
...
function Ff(t) {
	...
	return (
		...
		ne.node(o, o + i.nodeSize, {
			...
			[r]:
				typeof n == "function"
					? n({ editor: e, node: i, pos: o, hasAnchor: l })
					: n,
		})
	);
}
...
function mw(t) {
	...
	const e = [...t].sort((r, o) => r.from - o.from),
		n = [{ ...e[0] }];
	for (let r = 1; r < e.length; r += 1) {
		const o = n[n.length - 1],
			i = e[r];
		i.from <= o.to ? (o.to = Math.max(o.to, i.to)) : n.push({ ...i });
	}
	return n;
}
function gw(t, e) {
	const n = pw(t, e.from, e.to);
	return (
...
function yw(t, e, n) {
	...
	if (t.docChanged) {
		const o = Vn(t);
		for (const i of o) r.push(...gw(n.doc, i.newRange));
	}
	return (
		t.selectionSet &&
...
function ww({
	...
	let l = t;
	for (const a of e) {
		const { from: c, to: d } = bw(a.from, a.to, i),
...
function Sw({ editor: t, options: e }) {
	const n = e.dataAttribute ? `data-${kw(e.dataAttribute)}` : `data-${$f}`,
		r = e.showOnlyCurrent && !e.includeChildren;
	return new D({
		key: Pf,
		...(r ? {} : { state: xw({ editor: t, options: e, dataAttribute: n }) }),
		props: {
			decorations: r
				? ({ doc: o, selection: i }) =>
						Wf({
				...
				new D({
					...
					props: {
						decorations(n) {
							return Cw(n, t)
								? X.create(n.doc, [
										ne.inline(n.selection.from, n.selection.to, {
											class: e.className,
										}),
									])
								: null;
		...
		addKeyboardShortcuts() {
			return {
				...
				Tab: ({ editor: t }) => {
					...
					const n = (e = this.options.tabSize) != null ? e : da,
						{ state: r } = t,
						{ selection: o } = r,
						{ $from: i, empty: s } = o;
...
`),
									...
									const x =
											((a = h[p].match(/^ */)) == null ? void 0 : a[0]) || "",
										b = Math.min(x.length, n);
									if (b === 0) return !0;
									let w = d;
									for (let C = 0; C < p; C += 1) w += h[C].length + 1;
									return (
										l.delete(w, w + b),
										c - w <= b && l.setSelection(E.create(l.doc, w)),
										!0
									);
								})
							: t.commands.command(({ tr: l }) => {
...
`);
				...
				ArrowUp: ({ editor: t }) => {
					...
					const { state: e } = t,
						{ selection: n } = e,
						{ $from: r, empty: o } = n;
					if (!o || r.parent.type !== this.type || r.parentOffset !== 0)
						return !1;
					const i = r.before();
				...
				ArrowDown: ({ editor: t }) => {
					...
					return l === void 0
						...
						: r.nodeAt(l)
							? t.commands.command(
									({ tr: c }) => (c.setSelection(I.near(r.resolve(l))), !0),
								)
							: t.commands.exitCode();
				},
			};
...
`,
							...
							return (
								...
								s.selection.$from.parent.type !== this.type &&
									s.setSelection(
										E.near(s.doc.resolve(Math.max(0, s.selection.from - 2))),
									),
								s.setMeta("paste", !0),
								t.dispatch(s),
								!0
			...
			new D({
				props: {
					handleDrop(e, n) {
						if (
							!n ||
							(n.preventDefault(), !n.dataTransfer.getData("customBlock"))
						)
							return !1;
						const r = n.dataTransfer.getData("customBlock");
						return (
							t(r, e.posAtCoords({ left: n.clientX, top: n.clientY }).pos), !1
						);
					},
				},
			}),
...
var ii = (t, e) => {
		const n = e.view.domAtPos(t).node,
	...
	Xf = (t, e) => {
		...
		if (
			...
			return !1;
		const d = qe((y) => y.type === i.nodes.details)(s);
		if (!d) return !1;
		const u = fn(d.node, (y) => y.type === i.nodes.detailsContent);
		...
		addNodeView() {
			return ({ editor: t, getPos: e, node: n, HTMLAttributes: r }) => {
				...
				const c = (d) => {
					...
					const h = o.classList.contains(this.options.openClassName);
					l({ isOpen: h, node: f });
					const p = new Event("toggleDetailsContent"),
				...
				return (
					...
					s.addEventListener("click", () => {
						...
						if (t.isEditable && typeof e == "function") {
							const { from: d, to: u } = t.state.selection;
							t.chain()
								.command(({ tr: f }) => {
									const h = e();
			...
			return {
				setDetails:
					...
					({ state: t, chain: e }) => {
						...
						if (!r.nodes.detailsContent.contentMatch.matchFragment(a.content))
							return !1;
						const d = ((n = a.toJSON()) == null ? void 0 : n.content) || [];
						return e()
							.insertContentAt(
				...
				unsetDetails:
					...
					({ state: t, chain: e }) => {
						...
						if (!o) return !1;
						const i = fn(o.node, (y) => y.type === r.nodes.detailsSummary),
							s = fn(o.node, (y) => y.type === r.nodes.detailsContent);
		...
		addKeyboardShortcuts() {
			return {
				Enter: ({ editor: t }) => {
					...
					const y = o.node(-3);
					if (!y) return !1;
					const x = o.indexAfter(-3),
						b = un(y.contentMatchAt(x));
					if (!b || !y.canReplaceWith(x, x, b)) return !1;
					const w = b.createAndFill();
					if (!w) return !1;
					const { tr: k } = e,
						C = o.after(-2);
					k.replaceWith(C, C, w);
					const S = k.doc.resolve(C),
						R = I.near(S, 1);
					k.setSelection(R);
					const T = m,
						O = m + p.nodeSize;
					return k.delete(T, O), k.scrollIntoView(), n.dispatch(k), !0;
				},
			};
		},
			...
			insertGrid:
				...
				({ tr: r, dispatch: o, editor: i }) => {
					...
					for (let u = 0; u < l.length; u += 1)
						a.push(
							s.createAndFill({ "data-col-span": Number(l[u] ?? 1) || 1 }),
						);
					const c = l.map((u) => Number(u) || 1).reduce((u, f) => u + f, 0),
...
var nh = $.create({
	...
	parseHTML() {
		return [
			{
				tag: "div",
				getAttrs: (t) => t.classList.contains("grid-layout-col") && null,
			},
		];
	},
	renderHTML({ HTMLAttributes: t }) {
...
var Jw = $.create({
									...
									if (l) {
										...
										if (a.nodeAfter)
											a.nodeAfter.isTextblock
												? s.setSelection(E.create(s.doc, a.pos + 1))
												: a.nodeAfter.isBlock
													? s.setSelection(L.create(s.doc, a.pos))
													: s.setSelection(E.create(s.doc, a.pos));
										else {
											const d =
											...
											u &&
												(s.insert(c, u),
												s.setSelection(E.create(s.doc, c + 1)));
										}
										s.scrollIntoView();
									}
									return !0;
		...
		addInputRules() {
			return [Hr({ find: /^(?:---|—-|___\s|\*\*\*\s)$/, type: this.type })];
		},
	}),
	sh = Jw;
var Gw = /(?:^|\s)(\*(?!\s+\*)((?:[^*]+))\*(?!\s+\*))$/,
	...
	tx = $.create({
		...
		addNodeView() {
			...
			const {
				...
				o = new Set(["src", "width", "height"]);
			return ({ node: i, getPos: s, HTMLAttributes: l, editor: a }) => {
				const c = document.createElement("img");
				c.draggable = !1;
				const d = N(this.options.HTMLAttributes, l);
				...
				const h = (g) => {
						...
						const y = a.extensionManager.attributes.filter(
						...
						return (
							...
							Object.entries(x).forEach(([b, w]) => {
								o.has(b) ||
									(w != null ? c.setAttribute(b, w) : c.removeAttribute(b));
							}),
							f(x.src),
							(u = x),
							!0
					...
					p = new Zo({
						...
						getPos: s,
						onResize: (g, y) => {
							(c.style.width = `${g}px`), (c.style.height = `${y}px`);
						},
						onCommit: (g, y) => {
							const x = s();
							x !== void 0 &&
								this.editor
									.chain()
									.setNodeSelection(x)
									.updateAttributes(this.name, { width: g, height: y })
									.run();
						},
						onUpdate: h,
						options: {
		...
		addAttributes() {
			return {
				...
				width: {
					...
					renderHTML: (t) =>
						t.width ? { width: t.width, style: `width: ${ch(t.width)}` } : {},
				},
				height: {
					default: null,
					...
					renderHTML: (t) =>
						t.height
							? { height: t.height, style: `height: ${ch(t.height)}` }
							: {},
				},
			};
		},
		...
		if (
			...
			if (r)
				if (s.t && typeof s.t == "string") {
					const a = Object.assign(lx(s.t, r), n);
					gn(i, a, r);
				} else n && gn(i, n, r);
			s.t = i;
		}
...
function dx(t = []) {
	...
	si == null && (si = ph(nx)),
		...
		M(n, "\\", Ai),
		M(n, "\u30FB", xh);
	const r = oe(n, Ct, Ma, { [ya]: !0 });
	oe(r, Ct, r);
	const o = oe(r, St, bh, { [Wr]: !0 }),
		i = oe(r, _r, wh, { [Vr]: !0 }),
		s = oe(n, St, vt, { [ba]: !0 });
	oe(s, Ct, o), oe(s, St, s), oe(o, Ct, o), oe(o, St, o);
	const l = oe(n, _r, ka, { [wa]: !0 });
	oe(l, St), oe(l, Ct, i), oe(l, _r, l), oe(i, Ct, i), oe(i, St), oe(i, _r, i);
	const a = M(n, pa, Ta, { [ua]: !0 }),
		c = M(n, hh, va, { [ua]: !0 }),
		d = oe(n, ha, va, { [ua]: !0 });
	M(n, ma, d),
		M(c, pa, a),
...
function Ft(t, e, n, r, o) {
	...
	for (let l = 0; l < s - 1; l++) {
		const a = e[l];
		t.j[a] ? (i = t.j[a]) : ((i = new Ee(r)), (i.jr = o.slice()), (t.j[a] = i)),
			(t = i);
	}
...
vh.prototype = {
	...
	render(t) {
		const n = this.toHref(t.get("defaultProtocol")),
			...
			o = t.get("tagName", n, this),
			i = this.toFormattedString(t),
			s = {},
			l = t.get("className", n, this),
			a = t.get("target", n, this),
			c = t.get("rel", n, this),
			d = t.getObj("attributes", n, this),
			u = t.getObj("events", n, this);
		return (
			(s.href = r),
			l && (s.class = l),
			a && (s.target = a),
...
function px({ groups: t }) {
	...
	const p = M(u, Fe);
	M(p, Fe, p), U(p, t.domain, u), U(h, t.domain, u), M(h, it, f), M(h, Fe, p);
	const m = M(s, Fe),
		g = M(s, it);
...
function mx(t, e, n) {
	...
	for (; o < r; ) {
		...
		if (f < 0) (o -= d), o < r && (s.push(n[o]), o++);
			...
			s.length > 0 && (i.push(ga(gh, e, s)), (s = [])), (o -= f), (d -= f);
			const h = u.t,
				p = n.slice(o - d, o);
			i.push(ga(h, e, p));
		}
...
function Hi(t) {
	return Z.initialized || bx(), mx(Z.parser.start, t, Ch(Z.scanner.start, t));
}
Hi.scan = Ch;
		...
		appendTransaction: (e, n, r) => {
			...
			if (
				(Vn(l).forEach(({ newRange: c }) => {
					...
					if (u && f) {
						const h = f.split(wx).filter(Boolean);
						if (h.length <= 0) return !1;
						const p = h[h.length - 1],
							m = u.pos + f.lastIndexOf(p);
						if (!p) return !1;
						const g = Hi(p).map((y) => y.toObject(t.defaultProtocol));
						if (!Sx(g)) return !1;
						g.filter((y) => y.isLink)
							...
							.forEach((y) => {
								Pr(y.from, y.to, r.doc).some((x) => x.mark.type === t.type) ||
		...
		props: {
			handleClick: (e, n, r) => {
				...
				if (
					...
					const a = qo(e.state, t.type.name),
...
function Ox(t) {
	const e = ge({
		find: (n) => {
			var r, o;
			const i = [];
			for (const l of n.matchAll(Tx))
				Ah(n, l, t.isAllowedHref) && i.push(Eh(l));
			const s = (
				(o = (r = t.findPlainUrls) == null ? void 0 : r.call(t, n)) != null
					? o
					: []
	...
	return new D({
		...
		props: {
			handlePaste: (e, n, r) => {
				...
				const c = $i(a, { defaultProtocol: t.defaultProtocol }).find(
...
var Dx = ee.create({
					...
					getAttrs: (t) => {
						...
						return !e ||
							!this.options.isAllowedUri(e, {
								defaultValidate: (n) => !!Mt(n, this.options.protocols),
		...
		renderMarkdown: (t, e) => {
			var n, r, o, i;
			const s = (r = (n = t.attrs) == null ? void 0 : n.href) != null ? r : "",
...
`,
					...
					({ commands: t, chain: e }) =>
						this.options.keepAttributes
							? e()
									...
									.updateAttributes(Px, this.editor.getAttributes(Rh))
									.run()
							: t.toggleList(
									this.name,
									this.options.itemTypeName,
	...
	Bx = (t, e, n) => {
		...
		const { $from: o } = r;
		if (!o.parent.isTextblock || o.parentOffset !== o.parent.content.size)
			return null;
		let i = -1;
		for (let h = o.depth; h > 0; h -= 1)
			if (o.node(h).type.name === e) {
				i = h;
				break;
			}
		if (i < 0) return null;
		const s = o.node(i),
			l = o.index(i);
		if (l + 1 >= s.childCount) return null;
		const a = s.child(l + 1);
		if (!n.includes(a.type.name)) return null;
		let c = t.schema.nodes[e],
			d = !1;
		if (
			(a.forEach((h) => {
			...
			return null;
		const u = t.doc.resolve(o.after()).nodeAfter;
		if (!u || !n.includes(u.type.name)) return null;
		const f = [];
		return (
			u.forEach((h) => {
				f.push(h);
	...
	zx = (t, e, n, r) => {
		...
		return (
			d.insert(u, v.from(c)),
			d.setSelection(i.map(d.doc, d.mapping)),
			e && e(d),
			!0
		);
	},
...
function Vi(t) {
	let e = t,
		n = "";
	for (const [r, o] of zh) for (; e >= r; ) (n += o), (e -= r);
	return n;
}
function Ha(t) {
...
function Wx(t) {
	...
	const n = (e = Wi(t[0])) != null ? e : "numeric",
		r = $a(t[0]);
	if (r < 1) return !1;
		...
		parseMarkdown: (t, e) => {
			...
			if (t.tokens && t.tokens.length > 0) {
				if (qx(t))
					return {
						type: "listItem",
						content: [{ type: "paragraph", content: Jx(t.text || "", e) }],
					};
			...
			Wn(
				...
				(r) => {
					...
					if (r.parentType === "orderedList") {
						const a =
								((i = (o = r.meta) == null ? void 0 : o.parentAttrs) == null
									? void 0
									: i.start) || 1,
							c =
								(l = (s = r.meta) == null ? void 0 : s.parentAttrs) == null
									? void 0
									: l.type,
							d = a - 1 + (r.index || 0);
						return Kx(c, d, ". ");
...
var ji = (t, e) => {
	...
	Ia = (t, e, n) => {
		...
		if (!Ke(t.state, e) && Fh(t.state, e, n)) {
			const { $anchor: r } = t.state.selection,
			...
			const s = i.at(-1);
			if (!s) return !1;
			const l = t.state.doc.resolve(o.start() + s.pos + 1);
			return t
				.chain()
				...
				Delete: ({ editor: t }) => {
					...
					return (
						this.options.listTypes.forEach(({ itemName: n }) => {
							t.state.schema.nodes[n] !== void 0 && Pa(t, n) && (e = !0);
						}),
						e
					);
				},
					...
					return (
						this.options.listTypes.forEach(
							({ itemName: n, wrapperNames: r }) => {
								t.state.schema.nodes[n] !== void 0 && Ia(t, n, r) && (e = !0);
							},
						),
						e
					);
	...
	for (; n < t.length; ) {
		...
		for (; m < t.length; ) {
			...
			if (x.trim() === "") g.push(x), p.push(""), (y = !0), (m += 1);
				...
				const w = x.length - x.trimStart().length,
					k = d + l.length + 1;
...
`)
	...
	const n = [];
	for (const i of e) {
		const s = i.trim().match(sk);
		if (!s) return null;
		n.push({ marker: s[1], content: s[3] });
	}
	const r = n.map((i) => i.marker);
	return Wx(r)
		? {
				type: "orderedList",
				attrs: Ux(n[0].marker),
				content: n.map((i) => ({
					type: "listItem",
					content: [
						{ type: "paragraph", content: [{ type: "text", text: i.content }] },
					],
				})),
			}
		: null;
...
function Wh(t, e, n) {
	...
	for (; o < t.length; ) {
		const i = t[o];
		if (i.indent === e) {
			const { paragraphLines: s, blockLines: l } = ok(i.contentLines),
				a = s
					.join(`
`)
					.trim(),
...
var Va = $.create({
		...
		renderHTML({ HTMLAttributes: t }) {
			const { start: e, type: n, ...r } = t,
				o = N(this.options.HTMLAttributes, r);
			return (
				e !== 1 && (o.start = e), n && n !== "1" && (o.type = n), ["ol", o, 0]
			);
		},
		markdownTokenName: "list",
		parseMarkdown: (t, e) => {
			...
			return (
				...
				Object.keys(i).length > 0
					? { type: "orderedList", attrs: i, content: o }
					: { type: "orderedList", content: o }
			);
		},
		renderMarkdown: (t, e) =>
			t.content
...
`),
				...
				const a = Wh(s, s[0].indent, n);
				if (a.length === 0) return;
				const c = ((r = s[0]) == null ? void 0 : r.number) || 1,
					...
					({ commands: t, chain: e }) =>
						this.options.keepAttributes
							? e()
									...
									.updateAttributes(ck, this.editor.getAttributes(Lh))
									.run()
							: t.toggleList(
									this.name,
									this.options.itemTypeName,
				...
				new D({
					props: {
						handlePaste: (t, e) => {
							var n, r;
							const o =
								(n = e.clipboardData) == null ? void 0 : n.getData("text/html");
							if (o?.trim()) return !1;
							const i =
							...
							if (!s) return !1;
							try {
								const l = t.state.schema.nodeFromJSON(s),
									a = t.state.tr.replaceSelectionWith(l);
			...
			return ({ node: t, HTMLAttributes: e, getPos: n, editor: r }) => {
				const o = document.createElement("li"),
					...
					c = (u) => {
						...
						l.ariaLabel =
							((h =
								(f = this.options.a11y) == null ? void 0 : f.checkboxLabel) ==
							null
								? void 0
					...
					l.addEventListener("change", (u) => {
						...
						r.isEditable &&
							...
							!r.isEditable &&
								...
								(this.options.onReadOnlyChecked(t, f) ||
									(l.checked = !l.checked));
					}),
		...
		markdownTokenizer: {
			...
			tokenize(t, e, n) {
				const r = (i) => {
						...
						if (s) {
							const l = { type: "taskList", raw: s.raw, items: s.items },
								a = i.slice(s.raw.length);
	...
	hk = ({
		...
		return new D({
			...
			props: {
							...
							r().upload(`componentFileAttachments.${l}.${y}`, m, () => {
								c(y).then((x) => {
									x &&
										...
										g === f.length - 1 && Jn(d, "form-processing-finished"));
								});
							});
						}),
						!0
				...
				handlePaste(d, u) {
					const f = u.clipboardData?.files?.length
						...
						return m
							...
							: (u.preventDefault(),
								...
								f.forEach((g, y) => {
									...
									const x = ja();
									r().upload(`componentFileAttachments.${l}.${x}`, g, () => {
										c(x).then((b) => {
											b &&
												(t
													.chain()
					...
					if (p) {
						const g = new DOMParser().parseFromString(p, "text/html"),
							y = g.querySelectorAll("img[src]");
						if (y.length) {
							u.stopPropagation();
							const { from: x, to: b } = t.state.selection;
							return (
								Jn(d, "form-processing-started", { message: a }),
								t.setEditable(!1),
								d.dom.dispatchEvent(
								...
								(async () => {
									for (const S of y) {
										...
										if (!R.startsWith("data:image/")) continue;
										let T;
										try {
											const [V, W] = R.split(","),
												J = V.match(/:(.*?);/)[1],
										...
										const O = T.type.split("/")[1]?.split("+")[0] || "png",
											z = new File([T], `image.${O}`, { type: T.type });
										if (
											Wa({
												files: [z],
												acceptedTypes: e,
										...
										_ &&
											(S.setAttribute("src", _), S.setAttribute("data-id", B));
									}
									for (const S of Array.from(g.querySelectorAll("img[src]"))) {
										const R = g.createElement("p");
									...
									k.innerHTML = w;
									const C = Te.fromSchema(t.state.schema).parseSlice(k, {
										preserveWhitespace: !1,
									});
									t.view.dispatch(t.state.tr.replaceRange(x, b, C)),
										t.setEditable(!0),
										d.dom.dispatchEvent(
											new CustomEvent("rich-editor-uploaded-file", {
												bubbles: !0,
...
function Gh(t, e, n, r) {
	let o = _e(t),
		i = yk(De(t), n === "start", r);
	return (
		o && ((i = i.map((s) => s + "-" + o)), e && (i = i.concat(i.map(Qr)))), i
	);
}
function Zr(t) {
	const e = De(t);
...
async function Zh(t, e) {
	...
	return {
		top: (y.top - k.top + p.top) / w.y,
		bottom: (k.bottom - y.bottom + p.bottom) / w.y,
		left: (y.left - k.left + p.left) / w.x,
...
var np = (t) => (
		t === void 0 && (t = {}),
		{
			...
			async fn(e) {
				...
				if (l !== x) return { reset: { placement: m[0] } };
				const w = [g[De(x)], g[b[0]], g[b[1]]],
					k = [
						...(((r = s.autoPlacement) == null ? void 0 : r.overflows) || []),
						{ placement: x, overflows: w },
					],
					C = m[y + 1];
				if (C)
					return {
				...
				const S = k
						.map((O) => {
							const z = _e(O.placement);
							return [
								O.placement,
								z && d
									? O.overflows.slice(0, 2).reduce((F, B) => F + B, 0)
									: O.overflows[0],
								O.overflows,
							];
						})
					...
					T =
						((o = S.filter((O) =>
							O[2].slice(0, _e(O[0]) ? 2 : 3).every((z) => z <= 0),
						)[0]) == null
							? void 0
							: o[0]) || S[0][0];
				return T !== l
					? { data: { index: y + 1, overflows: k }, reset: { placement: T } }
					: {};
			},
		}
	),
		...
		t === void 0 && (t = {}),
		{
			...
			async fn(e) {
				...
				const y = De(o),
					x = Xe(l),
					b = De(l) === l,
					w = await (a.isRTL == null ? void 0 : a.isRTL(c.floating)),
					k = f || (b || !m ? [Zr(l)] : Jh(l)),
					C = p !== "none";
				...
				if (
					...
					if (
						...
						(!(u === "alignment" ? x !== Xe(V) : !1) ||
							O.every((Q) => (Xe(Q.placement) === x ? Q.overflows[0] > 0 : !0)))
					)
						return {
			...
			switch (o) {
				case "referenceHidden": {
					const s = await r.detectOverflow(e, {
						...
						l = Yh(s, n.reference);
					return {
						data: { referenceHiddenOffsets: l, referenceHidden: Qh(l) },
					};
				}
				case "escaped": {
					const s = await r.detectOverflow(e, { ...i, altBoundary: !0 }),
						l = Yh(s, n.floating);
			...
			async fn(e) {
				...
				function p() {
					...
					if (u.length >= 2) {
						...
						const g = De(n) === "left",
							y = fe(...u.map((O) => O.right)),
							x = Re(...u.map((O) => O.left)),
							b = u.filter((O) => (g ? O.left === x : O.right === y)),
							w = b[0].top,
	...
	ap = (t) => (
		t === void 0 && (t = {}),
		{
			...
			async fn(e) {
				...
				const g = a.fn({ ...e, [h]: p, [f]: m });
				return {
					...g,
					data: { x: g.x - n, y: g.y - r, enabled: { [h]: s, [f]: l } },
				};
			},
		}
	);
	...
	t === void 0 && (t = {}),
	{
		...
		async fn(e) {
			...
			let x = m - d.top - d.bottom,
				...
				R = k;
			if (
				((n = e.middlewareData.shift) != null && n.enabled.x && (R = b),
				(r = e.middlewareData.shift) != null && r.enabled.y && (S = x),
				C && !f)
			) {
				const O = fe(d.left, 0),
					z = fe(d.right, 0),
					...
					B = fe(d.bottom, 0);
				h
					? (R = p - 2 * (O !== 0 || z !== 0 ? O + z : fe(d.left, d.right)))
					: (S = m - 2 * (F !== 0 || B !== 0 ? F + B : fe(d.top, d.bottom)));
			}
...
function Ye(t) {
	var e;
	return (e = (up(t) ? t.ownerDocument : t.document) || window.document) == null
		? void 0
		: e.documentElement;
}
function up(t) {
...
function at(t) {
	return Gi() ? t instanceof HTMLElement || t instanceof Ne(t).HTMLElement : !1;
}
function dp(t) {
...
function Xn(t) {
	const { overflow: e, overflowX: n, overflowY: r, display: o } = We(t);
	return (
...
function At(t) {
	...
	const e = t.assignedSlot || t.parentNode || (dp(t) && t.host) || Ye(t);
...
function Ok(t) {
	let e = Ye(t),
		...
		l = -n.scrollTop;
	return (
		We(r).direction === "rtl" && (s += fe(e.clientWidth, r.clientWidth) - o),
...
var zk = async function (t) {
	const e = this.getOffsetParent || Sp,
...
function _k(t) {
	...
	let b = m + g.index,
		w = b + g[0].length;
	return (
		a && d.test(p.slice(w - 1, w + 1)) && ((g[0] += " "), (w += 1)),
		b < l.pos && w >= l.pos
			? { range: { from: b, to: w }, query: g[0].slice(n.length), text: g[0] }
			: null
	);
}
function Vk(t) {
...
function Wk(t) {
	return () => {
		const e = t.state.selection.$anchor.pos,
...
function Kk({ view: t, pluginKeyRef: e }) {
	const n = t.state.tr.setMeta(e, { exit: !0 });
	t.dispatch(n);
}
function qk({
	pluginKey: t,
		...
		handleKeyDown(l, a) {
			...
			return u.active
				? a.key === "Escape" || a.key === "Esc"
					...
					: ((d = i?.onKeyDown) == null
							? void 0
							: d.call(i, { view: l, event: a, range: u.range })) || !1
				: !1;
		},
		decorations(l) {
			const a = t.getState(l),
		...
		apply(u, f, h, p) {
			...
			if (k && k.exit)
				return (
					...
					(w.query = null),
					(w.text = null),
					(w.dismissedRange = f.active ? { ...f.range } : f.dismissedRange),
					w
				);
			if (
				((w.composing = g),
				u.docChanged &&
					w.dismissedRange !== null &&
					(w.dismissedRange = {
						from: u.mapping.map(w.dismissedRange.from),
						to: u.mapping.map(w.dismissedRange.to),
					}),
				m && (x || t.view.composing))
			) {
				(b < f.range.from || b > f.range.to) &&
					!g &&
					!f.composing &&
					(w.active = !1);
				const C = s({
				...
				C &&
				l({ editor: t, state: p, range: C.range, isActive: f.active }) &&
				(!a ||
					a({
						editor: t,
						range: C.range,
					...
					? (w.dismissedRange !== null &&
							!c({
								match: C,
								dismissedRange: w.dismissedRange,
								state: p,
								transaction: u,
							}) &&
							(w.dismissedRange = null),
						w.dismissedRange === null
							? ((w.active = !0),
								(w.decorationId = f.decorationId || S),
								(w.range = C.range),
								(w.query = C.query),
								(w.text = C.text))
							: (w.active = !1))
					: (C || (w.dismissedRange = null), (w.active = !1));
			} else w.active = !1;
			return (
				w.active ||
					((w.decorationId = null),
...
function Gk({ editor: t, items: e }) {
	let n = null,
		...
		o = null,
		i = () => {
			r !== null && (clearTimeout(r), (r = null)), o?.(), (o = null);
		},
		s = (c) =>
			new Promise((d) => {
				(o = d),
	...
	return {
		abort: l,
		fetch: async (c, d) => {
			l(), (n = new AbortController());
			const u = n;
			if ((d > 0 && (await s(d)), n !== u || u.signal.aborted))
				return { status: "aborted" };
			try {
				const f = await e({ editor: t, query: c, signal: u.signal });
				return n !== u || u.signal.aborted
					? { status: "aborted" }
					: { status: "resolved", items: f };
			} catch {
				return n !== u || u.signal.aborted
					? { status: "aborted" }
					: { status: "error" };
...
function Qk({
	...
	return (s, l = {}) => {
		...
		d && Yk(r).appendChild(s),
			l.onPosition ||
				((s.style.visibility = "hidden"), (s.style.width = "max-content"));
		let f = vp(
				a,
				s,
				() => {
					...
					}).then(({ x: p, y: m, placement: g, strategy: y }) => {
						if (l.onPosition) {
							l.onPosition({ x: p, y: m, placement: g, strategy: y });
							return;
						}
		...
		return (
			o &&
				((h = (p) => {
					const m = p.target;
					!(m instanceof Node) || s.contains(m) || e.contains(m) || i();
				}),
				document.addEventListener("pointerdown", h, !0)),
			() => {
				f(),
					h && document.removeEventListener("pointerdown", h, !0),
					d && s.remove();
			}
		);
	};
...
function Zk({
	...
	return {
		update: async (w, k) => {
			...
			const J = F === "stopped" ? O : z,
				Q = w.dom.querySelector(`[data-decoration-id="${J.decorationId}"]`),
				se = p(w, Q),
			...
			if (
				...
				F === "started" && ((R = r?.onBeforeStart) == null || R.call(r, g)),
				F === "updated" && ((T = r?.onBeforeUpdate) == null || T.call(r, g)),
				F === "started" && b(F, g),
				F === "started" || F === "updated")
			)
				if (!er) y.abort(), (g = { ...g, items: s ?? [], loading: !1 });
				else {
					(g = { ...g, items: s ?? [], loading: !0 }), (F = "updated"), b(F, g);
					const K = await y.fetch(J.query || "", i);
					if (K.status === "aborted") return;
					const dt = e.getState(w.state);
					if (!dt?.active) {
						y.abort();
						return;
					}
...
function tS({
	...
	const V = (W) => Kk({ view: W, pluginKeyRef: t });
	return new D({
		key: t,
		view: () =>
			Zk({
...
var ts = tS;
var nS = ({ editor: t, overrideSuggestionOptions: e, extensionName: n }) => {
		const r = new H();
		return {
			editor: t,
			char: "{{",
	...
	Op = $.create({
		...
		renderHTML({ node: t, HTMLAttributes: e }) {
			...
			const o = this.options.renderHTML({ options: r, node: t, suggestion: n });
				...
				new D({
					props: {
						handleDrop(t, e) {
							if (
								!e ||
								(e.preventDefault(), !e.dataTransfer.getData("mergeTag"))
							)
								return !1;
	...
	Dp = ({
		...
		return {
			items: async ({ query: l }) => {
				...
				return t.filter((c) => {
					const d = typeof c == "string" ? c : (c?.label ?? c?.name ?? "");
				...
				const u = () => {
					...
					f = () => {
						...
						if (((l.innerHTML = ""), d)) {
							...
							C.className = "fi-dropdown-header";
					...
					h = (b) => (b ? n : i ? r : e),
					p = (b, w) => {
						c && c.command({ id: b, label: w });
					},
					m = () => {
						if (!l || !c || (c.items || []).length === 0) return;
						const w = l.children[a];
						if (w) {
							const k = w.getBoundingClientRect(),
								C = l.getBoundingClientRect();
							(k.top < C.top || k.bottom > C.bottom) &&
					...
					x = () => {
						...
						const w = b[a],
							k =
								typeof w == "string"
									? w
									: (w?.label ?? w?.name ?? String(w?.id ?? "")),
							C = typeof w == "object" ? (w?.id ?? k) : k;
						p(C, k);
					};
				return {
					onStart: (b) => {
					...
					onUpdate: (b) => {
						(c = b), (a = 0), f(), m(), b.clientRect && Rp(b.editor, l);
					},
					onKeyDown: (b) =>
						b.event.key === "Escape"
...
var rS = ({ editor: t, overrideSuggestionOptions: e, extensionName: n }) => {
		const r = new H(),
			o = e?.char ?? "@",
			i = e?.extraAttributes ?? {};
		return {
		...
		addProseMirrorPlugins() {
			const t = async (e) => {
				...
				if (typeof i == "function")
					try {
						const s = o.map(({ id: a, char: c }) => ({ id: a, char: c })),
							l = await i(s);
			...
			return [
				...this.storage.suggestions.map(ts),
				new D({
					view: (e) => (setTimeout(() => t(e), 0), { update: (n) => t(n) }),
				}),
			];
		},
		onBeforeCreate() {
					...
					c = {
						...Dp({
							items: async ({ query: h }) => {
								...
								return m.filter((y) => {
									const x =
										typeof y == "string" ? y : (y?.label ?? y?.name ?? "");
									return String(x).toLowerCase().includes(g);
								});
							},
							isSearchable: l,
							noOptionsMessage: i,
...
var Y = class Et extends I {
	...
	replace(e, n = A.empty) {
		...
		const i = I.findFrom(e.doc.resolve(e.mapping.slice(r).map(this.to)), -1);
		i && e.setSelection(i);
	}
	replaceWith(e, n) {
		this.replace(e, new A(v.from(n), 0, 0));
	}
	forEachCell(e) {
...
function lc(t, e) {
	...
	return e ? e.doc != t.doc && Qp(e.doc, t.doc, 0, r) : t.doc.descendants(r), n;
}
function vS(t, e, n, r) {
	const o = ie.get(e);
...
function Zp(t, { map: e, tableStart: n, table: r }, o) {
	...
	for (let s = 0; s < e.height; s++) {
		...
		if (o > 0 && o < e.width && e.map[l - 1] == e.map[l]) {
			...
			const a =
					i == null ? Se(r.type.schema).cell : r.nodeAt(e.map[l + i]).type,
...
function TS(t, e, n) {
	var r;
	const o = Se(e.type.schema).header_cell;
	...
	if (e) {
		...
		if (
			(i.setNodeMarkup(a + r.tableStart, null, {
				...Xp(c.attrs, c.attrs.colspan, r.right - r.left - c.attrs.colspan),
...
function DS(t) {
	...
	if (i == "row")
		for (let a = 0; a < e.childCount; a++) {
			...
			(d || u) && (c = oc(Se(s).row, new A(c, d, u)).content), l.push(c);
		}
...
function LS(t, e) {
	...
	for (let o = 0; o < n.length; o++)
		if ((o >= e.length && e.push(v.empty), n[o] < r)) {
			const i = Se(t).cell.createAndFill(),
				s = [];
...
function Wp(t, e, n, r, o, i, s, l) {
	...
	for (let c = o; c < i; c++) {
		...
		if (e.map[d - 1] == u) {
			...
			t.setNodeMarkup(p, null, Cn(f.attrs, s - h, f.attrs.colspan - (s - h))),
				t.insert(p + f.nodeSize, f.type.createAndFill(Cn(f.attrs, 0, s - h))),
				(c += f.attrs.rowspan - 1);
		}
	}
	return a;
...
function HS(t, e, n) {
	...
	if (o instanceof Y) {
		...
		const i = o.$anchorCell.node(-1),
			...
			l = ie.get(i).rectBetween(o.$anchorCell.pos - s, o.$headCell.pos - s);
		return (
		...
		const i = cs(t.state),
			s = i.start(-1);
		return (
			jp(t.state, t.dispatch, s, ie.get(i.node(-1)).findCell(i.pos - s), r), !0
		);
	} else return !1;
}
function $S(t, e) {
	...
	function s() {
		t.root.removeEventListener("mouseup", s),
			...
			jt.getState(t.state) != null && t.dispatch(t.state.tr.setMeta(jt, -1));
	}
	function l(a) {
		let c = a,
	...
	const i = new D({
		...
		state: {
			init(s, l) {
				...
				return r && c && (c[d] = (u, f) => new r(u, n, f)), new _S(-1, !1);
			},
			apply(s, l) {
				return l.apply(s);
			},
...
var _S = class as {
	...
	apply(e) {
		const r = e.getMeta(Le);
		if (r && r.setHandle != null) return new as(r.setHandle, !1);
		if (r && r.setDragging !== void 0)
			return new as(this.activeHandle, r.setDragging);
		if (this.activeHandle > -1 && e.docChanged) {
			let o = e.mapping.map(this.activeHandle, -1);
			return rc(e.doc.resolve(o)) || (o = -1), new as(o, this.dragging);
		}
		return this;
	}
};
...
function qS(t, e, n) {
	...
	for (let c = 0; c < i.height; c++) {
		...
		const p = f.colwidth ? f.colwidth.slice() : JS(f.colspan);
...
function XS(t) {
	const e = (t.style.textAlign || "").trim().toLowerCase(),
...
function wm(t) {
	...
	if (!e)
		throw new Error(
			`[tiptap error]: "${t.name}" has no default content to backfill.`,
		);
	return e.content;
}
var e1 = $.create({
...
var r1 = class {
	constructor(t, e, n, r = {}) {
		...
		for (const [o, i] of Object.entries(r))
			i != null &&
				(o === "style"
					? (this.table.style.cssText = String(i))
					: this.table.setAttribute(o, String(i)));
...
function c1(t) {
	...
	for (; n < t.length; ) {
		...
		for (; o < t.length; ) {
			...
			let s = 0;
			for (; o + s < t.length && t[o + s] === "`"; ) s += 1;
			if (s === r) {
				const l = t.slice(n + r, o);
				(e +=
					t.slice(n, n + r) +
					l.replace(/(?<!\\)\|/g, "\\|") +
					t.slice(o, o + r)),
...
function d1(t) {
	return t
		.split(`
`)
		.map((e) => (!e.includes("|") || !e.includes("`") ? e : c1(e)))
		.join(`
`);
}
function u1(t) {
...
function f1(t, e, n = {}) {
	var r;
	const o = (r = n.cellLineSeparator) != null ? r : a1;
...
`)
	...
	const s = i.reduce((m, g) => Math.max(m, g.length), 0);
	if (s === 0) return "";
	const l = Array.from({ length: s }).fill(0);
	i.forEach((m) => {
		var g;
		for (let y = 0; y < s; y += 1) {
			const b = (((g = m[y]) == null ? void 0 : g.text) || "").length;
			b > l[y] && (l[y] = b), l[y] < 3 && (l[y] = 3);
		}
	});
	const a = (m, g) => m + " ".repeat(Math.max(0, g - m.length)),
		c = i[0],
	...
	let f = `
`,
		h = Array.from({ length: s }).map((m, g) => (d && c[g] && c[g].text) || "");
	return (
		(f += `| ${h.map((m, g) => a(m, l[g])).join(" | ")} |
`),
		(f += `| ${l
			.map((m, g) => {
				const y = Math.max(3, m),
					x = u[g];
				return x === "left"
					? `:${"-".repeat(y)}`
					: x === "right"
		...
		parseMarkdown: (t, e) => {
			...
			if (t.header) {
				...
				t.header.forEach((i, s) => {
					var l;
					const a = us((l = r[s]) != null ? l : i.align),
						c = a ? { align: a } : {};
					o.push(
			...
			return (
				t.rows &&
					t.rows.forEach((o) => {
						...
						o.forEach((s, l) => {
							var a;
							const c = us((a = r[l]) != null ? a : s.align),
								d = c ? { align: c } : {};
							i.push(
...
`);
		...
		addCommands() {
			return {
				...
				setCellSelection:
					...
					({ tr: e, dispatch: n }) => {
						if (n) {
							const r = Y.create(e.doc, t.anchorCell, t.headCell);
							e.setSelection(r);
						}
						return !0;
					},
		...
		addNodeView() {
			const t = this.options.resizable && this.editor.isEditable,
				e = this.options.View;
			return t || !e
				? null
				: ({ node: n, view: r, HTMLAttributes: o }) => {
						const i = N(this.options.HTMLAttributes, o);
	...
	xm = j.create({
		...
		addExtensions() {
			const t = [];
			return (
				this.options.table !== !1 && t.push(p1.configure(this.options.table)),
	...
	Mm = ({ mergeTags: t, noMergeTagSearchResultsMessage: e }) => ({
		...
		render: () => {
			...
			return {
				...
				onUpdate: (f) => {
					(o = f), (r = 0), s(), a(), f.clientRect && vm(f.editor, n);
				},
				onKeyDown: (f) =>
					f.event.key === "Escape"
...
var Tm = async ({
	...
	const S = [
		...
		R = await Promise.all(
			o.map(async (T) => {
				...
				try {
					const z = (await import(T)).default;
	...
	for (let T of R) {
		...
		const O = S.findIndex((z) => z.name === T.name);
		T.name === "placeholder" &&
			T.parent === null &&
...
var w1 = class {
		constructor({
			...
			(this.preventHide = !1),
				...
				(this.shouldShow = ({ view: u, state: f, from: h, to: p }) => {
					...
					return !(!(u.hasFocus() || b) || y || x || !this.editor.isEditable);
				}),
				...
				(this.handleDebouncedUpdate = (u, f) => {
					const h = !f?.selection.eq(u.state.selection),
		...
		get virtualElement() {
			...
			if (
				!(
					(n = (e = this.view) == null ? void 0 : e.dom) != null && n.parentNode
				)
			)
				return;
			...
			if (r instanceof Y) {
				...
				const h =
					u === f
						...
						: b1(u.getBoundingClientRect(), f.getBoundingClientRect());
...
function S1({
	...
	return {
		...
		async init() {
			...
			const W = "paragraph" in l;
			Object.keys(l).forEach((K) => {
				const dt = this.$refs[`floatingToolbar::${K}`];
				if (!dt) {
				...
				B.registerPlugin(
					fc({
						...
						shouldShow: ({ editor: Ut }) =>
							K === "paragraph"
								? Ut.isFocused && Ut.isActive(K) && !Ut.state.selection.empty
								: W && !Ut.state.selection.empty && Ut.isActive("paragraph")
									? !1
		...
		setEditorSelection(W) {
			W &&
				...
				B.chain()
					.command(
						({ tr: J }) => (
							J.setSelection(k1.fromJSON(B.state.doc, this.editorSelection)), !0
						),
					)
					.run());
		},
		runEditorCommands({ commands: W, editorSelection: J }) {
			...
			let Q = B.chain();
			W.forEach((se) => (Q = Q[se.name](...(se.arguments ?? [])))), Q.run();
		},
		togglePanel(W = null) {
