...
export default function RoasteryShow({ roastery, beans }) {
	...
	const canEdit =
		auth.user &&
		(auth.user.role === "admin" || auth.user.id === roastery.created_by);
				...
				<div className="grid gap-8 lg:grid-cols-[1fr_300px]">
					<div>
						...
						<p className="mt-3 max-w-xl text-[15px] text-mocha">
							Semua bean dari roastery ini, dalam satu tempat. Ulasan dan resep
							dari komunitas, supaya kamu bisa menilai sebelum membeli.
						</p>
					...
					<Card className="h-fit p-5">
						...
						<dl className="mt-3 space-y-3 text-sm">
							...
							<div>
								...
								<dd className="mt-0.5 text-espresso">{beans.total} bean</dd>
							</div>
						</dl>
					</Card>
				</div>
