		...
		<AppLayout>
			<section className="py-10 lg:py-14">
				<h1 className="max-w-2xl text-display">
					Rumah bagi pencinta kopi specialty
				</h1>
				<p className="mt-3 max-w-xl text-[15px] text-mocha">
					Bagaimana rasa bean ini menurutmu? Bagikan metode seduh, catatan rasa,
					ukuran gilingan — dan temukan resep yang paling cocok untuk bean-nya
				...
				<section>
					...
					{beans.data.length === 0 ? (
						<EmptyState
							title={
								search
									? "Tidak ada bean yang cocok dengan pencarianmu"
									: "Belum ada bean"
							}
							message={
								search
				...
				<aside className="space-y-5">
					<div className="rounded-lg bg-gradient-to-br from-brown to-espresso p-6 text-center">
						<h3 className="text-[20px] text-bg">
							Pernah coba bean yang layak diceritakan?
						</h3>
						<p className="mt-2 text-[13px] text-bg/80">
							Bagikan ulasan, resep, atau keduanya. Komunitas menyeduh lebih
							baik lewat catatanmu.
						</p>
						{auth.user ? (
							auth.user.email_verified_at ? (
								<Link
									href={beanRoutes.create.url()}
