		...
		<div className="card-surface p-6">
			...
			<dl className="mt-4 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
				{specs.map(([label, value]) => (
					<div
						key={label}
						className="flex items-baseline justify-between gap-3 rounded-md bg-card/60 px-3.5 py-2.5"
					>
						<dt className="eyebrow">{label}</dt>
						<dd className="text-right text-sm font-semibold text-espresso">
							{value || "—"}
						</dd>
					</div>
				))}
			...
			{bean.flavour_perception && (
				<div className="mt-5">
					...
					<p className="mt-1.5 text-[14px] leading-relaxed text-espresso">
						{bean.flavour_perception}
					</p>
				</div>
			)}
...
export default function BeanShow({
	bean,
	tab,
	canWrite,
	canEdit,
	...
	const { auth } = usePage().props;
	const rating = bean.reviews_avg_rating
		? Number(bean.reviews_avg_rating).toFixed(1)
		: null;
		...
		<AppLayout>
			<nav aria-label="Navigasi breadcrumb" className="pt-6">
				<ol className="flex items-center gap-1.5 text-[12px] text-mocha">
					<li>
						<Link href="/roasters" className="hover:text-brown">
							Roaster
						</Link>
					</li>
					...
					<li>
						<Link
							href={
								bean.roastery
									? roasteries.show.url(bean.roastery.id)
									: "/roasters"
							}
						...
						<div className="mt-3 flex flex-wrap items-center gap-3">
							{rating ? (
								<span className="flex items-center gap-2">
									...
									<span className="text-[13px] text-mocha">
										· {bean.reviews_count} ulasan
									</span>
								</span>
							) : (
							...
							{bean.roast_level && (
								<Pill
									variant={roastVariant[bean.roast_level.name] ?? "neutral"}
								>
							...
							{bean.purpose && (
								<Pill variant="neutral">Untuk {bean.purpose.name}</Pill>
							)}
						</div>
					</div>
					{canEdit && (
				...
				<div className="mt-8 grid gap-8 lg:grid-cols-[1fr_300px]">
						...
						>
							{tabs.map((name) => (
								...
								>
									{tabLabels[name]}
									{name === "reviews" && (
										<span className="ml-1.5 text-[12px] text-mocha">
											({bean.reviews_count})
										</span>
									)}
									{name === "recipes" && (
					...
					<aside className="space-y-5">
						<Card className="p-5">
							...
							{bean.roastery && (
								<Link
									href={roasteries.show.url(bean.roastery.id)}
						...
						{!auth.user && (
							<div className="rounded-lg bg-gradient-to-br from-brown to-espresso p-6 text-center">
								<h3 className="text-[20px] text-bg">
									Catat perjalanan kopimu sendiri
								</h3>
								<p className="mt-2 text-[13px] text-bg/80">
									Daftar untuk menulis ulasan, membagikan resep, dan mendukung
									yang menurutmu oke.
								</p>
