...
export default function GateBanner({ action, message, cta, redirectTo, variant = 'guest' }) {
    ...
    return (
        ...
        >
            ...
            <p className="mx-auto mt-2 max-w-md text-[13.5px] text-bg/80">{message}</p>
            <button type="button" onClick={go} className="btn-primary mt-5 bg-caramel hover:bg-caramel-hover">
                {ctaLabel}
            </button>
        </section>
    );
